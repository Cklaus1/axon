...
8. **R10 — self-improving harness.** Spec ✅ Reviewed. ⚠️ R10→55%: **two safety-critical gates BUILT** (this commit, `improve.rs`) — G1 correctness (interpreter oracle over the whole corpus, observable tuple, E1401; overfit caught structurally; AI-judgment forbidden by construction E1406) + G2 capability-diff (I-12, E1402 via new `program_capabilities` collector). Deterministic. Added `run_program_capturing` (in-process stdout sink) for the G1 oracle. Remaining (impl under same spec, no new spec needed): G3 regression gate (E1403), G4 native timing harness, `axon improve` CLI + multi-sig graduation manifest (E1404/E1405).
