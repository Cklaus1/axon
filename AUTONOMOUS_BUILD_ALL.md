...
6. **R6 — capability/registry.** Spec ✅ Reviewed. ⚠️ R6→38%: import-edge boundary VERIFIED (f34af59); **lockfile FOUNDATION built** (this commit) — `lockfile.rs` zero-dep: `axh1:` SHA-256 + deterministic `axon.lock` TOML round-trip + E1205/E12xx band. TOML decided: hand-rolled minimal (no `toml` crate). Remaining slices: (a) `axon lock`/`add` CLI writes axon.lock from a program's `use`s; (b) import-time `--locked` hash check (E1201/E1202) wired into cmd_check; (c) E1203 cap-widening; (d) audit pinning (needs R3). **← NEXT: `axon lock` CLI + --locked check**
