...
6. **R6 — capability/registry.** Spec ✅ Reviewed. ⚠️ R6→30%: import-edge boundary VERIFIED (this commit) — static cap checker catches a `mod`+`use`-imported fn's `@[contained]`/`never:` violation (E1004) on the merged program, the I-11 floor the lockfile builds on. Remaining (the bulk): `axh1:` SHA-256 + `axon.lock` + `axon add`/`--locked` tamper check (E1201) + import audit. Needs a `toml` dep decision. **← lockfile machinery is the next real build**
