...
5. **R3 — AI primitive.** Spec ✅ Reviewed. ⚠️ R3→55%: AiCall provenance record DONE (01d0f9a); OFFLINE FALLBACK + E1300 DONE (this commit) — `@[ai(policy(fallback:…))]` parses (nested+flat) and returns Ok(fallback) mode:"fallback" offline, else E1300. Demo `examples/asi/ai_fallback.ax`. Remaining: `tier:` named-arg routing (tier→model table), budget gate E1301 (Phase-7), `axon ai policy` CLI. **← NEXT slice: tier routing OR move to R6**
6. **R6 — capability/registry.** Spec ✅ Reviewed. ⚠️ R6→48%: import-edge boundary VERIFIED (f34af59); lockfile foundation (2f542f1); **`axon lock` + `verify-lock` CLI WIRED** (this commit) — content-addressed tamper detection works end-to-end: lock writes axon.lock, verify-lock rejects a tampered module E1201 / missing entry E1202. TOML hand-rolled (no `toml` crate). Remaining slices: (a) `--locked` flag on `axon check` (verify-lock already gates the same); (b) E1203 import cap-widening; (c) AI audit pinning (needs R3 routing); (d) transitive-`use` hashing (currently direct edge only). **← NEXT: R7 wasm Slice A, or R6 --locked/E1203 follow-ons**
7. **R7 — targets.** Spec ✅ Reviewed. ⚠️ R7→35%: **Slice A DONE** (this commit) — interpreter runs on `wasm32-wasip1` via `wasmtime`, 15/15 pure-compute examples match native exit+stdout (`scripts/wasm_parity.sh`). One touchpoint changed: `on_deep_stack` cfg(wasm32) on the single stack; 64 MiB wasm stack (`.cargo/config.toml`) + `RECURSION_LIMIT=450` → graceful deep-recursion panic, no trap. WASI chosen over unknown-unknown (free fs/env). Remaining: `AxonHost` trait (unknown-unknown/browser), `axon target` CLI, js/mobile, AOT-wasm Slice B. Tooling installed: wasm32-wasip1/unknown-unknown targets, wasmtime, wasm-bindgen, wasm-pack.
8. **R10 — self-improving harness.** Spec ✅ Reviewed. ⚠️ R10→70%: **ALL FOUR GATES BUILT** (`improve.rs`). 0ffd35e: G1 correctness (interpreter oracle, whole corpus, E1401; overfit structural; AI-judgment forbidden E1406) + G2 capability-diff (I-12, E1402, `program_capabilities` collector) + `run_program_capturing` stdout sink. This commit: G3 regression (each member's @[test] outcome preserved, E1403 — catches what G1's main-run misses) + G4 perf (opt-in `measure_perf` wall-clock, faster/NotFaster/Unmeasured, W1410 — advisory claim-check). Remaining (impl under same spec): `axon improve` CLI (discover/verify/graduate/revert/list) + content-addressed pass manifest + multi-sig graduation (E1404/E1405) — the graduation machinery atop a complete verification core.
