...
| crate axon-gfx-mock | ? | ? | ✗ | ? | NOT YET ASSESSED. This row exists so the crate's absence from the manifest is visible rather than silent — every value here is a placeholder, not a finding. The manifest was hand-authored from one session's work and covered 3 of 21 crates; a crate-coverage check now makes that measurable. |
| crate axon-guest-kernel | ~ | ✗ | ✗ | ✓ | ASSESSED. A real freestanding kernel (PVH boot, SYSCALL gate, deny-all bitmask) with hypercall.rs an empty TODO stub — while main.rs unconditionally prints '[axon-kernel] hypercall substrate active', so a boot transcript reads as if it shipped. All four exercising scripts are manual-only. AXON_KERNEL.md cites qemu_boot_test.sh as proof; that script never references this crate or its target. |
...
21 of 21 crates are represented (0 explicitly excused). 22 of 52 subsystems have a production proof; 8 are UNKNOWN — not failing, unestablished, which is the state most worth acting on.
