...
- **Fix:** Widened to IO/FS with an EXEMPT_IO list where every entry carries a reason, and both directions now ask `capability_of_builtin_multi`. A stale-exemption assertion fails if a listed name later becomes classified. KNOWN GAP recorded rather than closed: the goal_* family appends program-derived lines to $XDG_CACHE_HOME/axon/provenance.jsonl from inside @[contained(fs: [])] (exit 0) and past a scoped sandbox with fs_write deny-all. Exempted because the path comes from the environment and never from program input, and classifying goal_* as fs:write would refuse every contained optimiser — a policy change, not a bug fix. The fixed-path property is pinned by the_provenance_path_is_not_program_addressable, so the exemption's justification fails loudly if it stops holding. Residual risk: a low-bandwidth covert channel into the same user's cache. (`ed1e30a`)
