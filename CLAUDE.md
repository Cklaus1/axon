...
| 14 | ✅ Complete | **Distributed types — both slices landed.** Slice 1 (`crdt.ax`): `GCounter`, `PNCounter`, `LWWRegister`, `VectorClock` — 15 `@[test]`s. Slice 2 (`replicated.ax`): `Replicated<T>` (eventually-consistent replicated register via LWW; `replicated_write/sync/converge/read/converged`) and `Quorum<T>` (majority-quorum linearizable writes; `quorum_propose/ack/try_commit/read`) — 17 `@[test]`s. **Headline contracts**: convergence (after `replicated_converge` all N nodes agree regardless of write order) and linearizable safety (a quorum write commits iff ≥⌊n/2⌋+1 replicas ack; a stale proposal at ts ≤ committed_ts is rejected). Together these realise `Replicated<T>`, `Quorum<T>`, and `CRDT<T>` as Tier-2 stdlib Store constructors (ROADMAP §6). |
