...
| 5 | 🚧 In progress | **Landed (no Z3):** `@[pure]` purity checker (E1207), `@[total]` termination checker (E1208), and refinement types `T where <pred>` — named + inline (bare param `d: i64 where _!=0` and parenthesized `-> (T where P)`), transparent to base, with **constant-predicate obligations** at arg/return/struct-field sites (E1209; predicate subset: arith, comparisons, `&&`/`\|\|`/`!`, `str_len`, `str_eq`). **NON-constant refinement contracts are enforced at RUNTIME at ALL FOUR obligation sites** (the spec's Z3-free `--proof-timeout 0` fallback): PRECONDITIONS (param `p: T where P` at fn entry), POSTCONDITIONS (fn `-> T where P` at each return), STRUCT CONSTRUCTION (refined fields + whole-struct `where _.lo<=_.hi`, `_`=instance), and LET/OWN/REF bindings (`let p: T where P`, plus the constant case now a static E1209); a violation exits **6** (`REFINE_VIOLATION_EXIT_CODE`); enforced in BOTH interp + native codegen (byte-identical, `exit_code_parity.sh`), out-of-subset codegen cases E0910-refused. The SMT backend (`smt.rs`, opt-in `smt` feature, `axon verify`) statically discharges what it can prove (@[verify] bounds, refinement returns, arg-forwarding subtyping), and **SMT discharge is now wired into the DEFAULT pipeline** (514e059) — proven for-all-inputs `@[verify]`/refinement-return checks are statically elided at run/build, not just under `axon verify`. **Remaining:** struct whole-refinement static discharge (`where _.lo<=_.hi` class); widen the proven-∀-inputs set beyond scalar `@[verify]` + refinement-return. See `spec/compiler-phase5.md` |
| 6 | 🚧 In progress | Row-polymorphic effects + handlers. **Landed:** effect-row syntax `fn f() -> T \| {IO, Net}` (parse + fmt); builtin effect catalog (`builtin_effect_row`); subsumption checker (E1310) with **transitive** anti-laundering (no hiding an effect behind an un-annotated helper); `@[contained]`→effect-row bridge; `substrate`/`surface` file markers (E1306); the effect walker recurses `with`/`for`/assign bodies (no laundering hole); **handler discharge (E04)** for inline *and* named handlers (`handler NAME = handler { on E(p)=>… }` defs resolve via parser desugar). Effect codes are **E1306 + the E131x block** (the spec's nominal E1300–E1308 collided with AI-policy E1300–E1302). **Remaining:** row-variable unification (E03, HM-level — open rows are currently conservative), and a **true suspend-across-host-event continuation** — replay-based multi-shot `resume` landed (7a79772, E1314; the bare-tail fast path is lowered to codegen, the rest is interp-only); the remaining gap is a native async suspend (the interp has no `Flow::SuspendForHost`, the scheduler no `FiberState::Suspended`), which is what gates R13/R7c/R14 interactive targets. See `spec/compiler-phase6.md` |
| 7 | 🚧 Mostly complete | `Principal` authority registry, cooperative scheduler, live `Supervisor`, durable `Store<T,C>`, `LLM<Caps>` gateway with per-token cost metering — R12 slices 1–5 (3aae955). **Gap:** kernel `Goal<M>` is NOT built — only the `LLM<Caps>` half of slice 5 shipped (`kernel.rs:485` is a comment stub + `LlmGateway`); no `KernelGoal` struct / principal-scoped goal builtins yet |
| 8 | ✅ Complete | `for!` search + `goal{}` block desugar to `goal_run` at parse time (221a5d0); `agent{}`/`search` underspecified, deferred |
| 9 (Layer-3) | 🚧 Prototype | Self-improving compiler: AI-authored passes as DATA (`RewriteSpec` DSL); firewall rejects unsound paths; `fold-logical` rule kind red-teamed (983395f/3da3509/044b73e) |
