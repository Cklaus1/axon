...
| 5 | 🚧 In progress | **Landed (no Z3):** `@[pure]` purity checker (E1207), `@[total]` termination checker (E1208), and refinement types `T where <pred>` — named + inline (bare param `d: i64 where _!=0` and parenthesized `-> (T where P)`), transparent to base, with **constant-predicate obligations** at arg/return/struct-field sites (E1209; predicate subset: arith, comparisons, `&&`/`\|\|`/`!`, `str_len`, `str_eq`). **NON-constant refinement preconditions are enforced at RUNTIME** (the spec's Z3-free `--proof-timeout 0` fallback): the param's predicate is evaluated at fn entry with `_`=arg, a violation exits **6** (`REFINE_VIOLATION_EXIT_CODE`); enforced in BOTH interp + native codegen (byte-identical, `exit_code_parity.sh`), out-of-subset codegen cases E0910-refused. **Remaining:** §4 Z3/SMT backend (`verify.rs::prove()`) for STATIC discharge of non-constant cases. See `spec/compiler-phase5.md` |
