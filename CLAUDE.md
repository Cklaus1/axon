...
| 13 | 🚧 Slice 1 landed | **Probabilistic refinement fragment — Slice 1.** Three tractable distribution families (Gaussian, Beta, Categorical) as userland types (`examples/stdlib/distribution.ax`) and `Belief` for discrete Bayesian state (`examples/stdlib/belief.ax`). **11 interpreter builtins landed:** pure moment/CDF builtins — `gaussian_pdf`, `gaussian_cdf`, `beta_mean`, `beta_variance`, `beta_cdf`, `categorical_mean`, `categorical_variance`, `categorical_cdf`; impure sampling builtins (Random effect row) — `gaussian_sample`, `beta_sample`, `categorical_sample`. Sampling uses Box-Muller (Gaussian), Marsaglia-Tsang via Gamma ratio (Beta), and linear-search CDF inversion (Categorical). CDF uses Abramowitz & Stegun erf approximation (Gaussian) and Lentz continued-fraction regularized incomplete beta (Beta). 15 distribution tests + 10 belief tests pass. Spec: `spec/compiler-phase13.md`. **Remaining (Slice 2):** `E[dist]`, `Var[dist]`, `P(dist <= k)` syntax in refinement predicates, discharged via interval arithmetic for constant distribution parameters. |
