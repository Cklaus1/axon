...
| 12 | 🚧 In progress | **Web UI — Goal Approval Flow.** `crates/axon-web` (`axon-web` binary): minimal synchronous HTTP server (`tiny_http`) that is a thin JSON proxy over the Phase-10 CLI commands. API: `POST /api/intent/compile`, `/api/ast/review`, `/api/ast/approve`, `/api/redteam`, `/api/deploy`, `GET /api/trace` — each runs `axon <verb> --json`, returns the CLI JSON verbatim or wraps into `{ok, exit_code, stdout, stderr}`. `GET /` serves an embedded single-page HTML/JS approval-flow UI with 6 panes (Intent → AST Review → Approve → Redteam → Deploy → Trace). 7 tests pass (`cargo test -p axon-web`). Spec: `spec/compiler-phase12.md`. **Remaining:** end-to-end browser test with a live `axon` binary; pane state-machine polish (disable buttons until prior step completes). |
