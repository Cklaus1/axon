...
| 8 | ✅ Complete | `for!` search + `goal{}` block desugar to `goal_run` at parse time (221a5d0); `goal { … subject_to: "fn" }` desugars to `goal_run_constrained` (hard feasibility gate); `agent{}`/`search` underspecified, deferred |
