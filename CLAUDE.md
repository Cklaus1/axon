...
| 14 | 🚧 Slice 1 | **Distributed types (CRDT + VectorClock) — Slice 1 landed.** `examples/stdlib/crdt.ax`: four conflict-free replicated data types — `GCounter` (grow-only counter; merge = component-wise max), `PNCounter` (positive-negative counter built from two GCounters), `LWWRegister` (last-write-wins register; tie-break = higher value), and `VectorClock` (causal ordering; `vc_happens_before`, `vc_concurrent`, `vc_merge`). Headline contract: **convergence** — any two replicas that have seen the same updates converge to identical state regardless of delivery order, with no coordination. 15 `@[test]`s pass. These are the userland realizations of `CRDT<T>` (ROADMAP §6 Tier-2); `Replicated<T>` and `Quorum<T>` remain (Slice 2). |
