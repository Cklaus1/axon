...
| 9 | 🚧 In progress | **Replay, audit, sandbox.** Slice 1 LANDED: every `axon run` / `axon goal` stamps a `run_start` event to the provenance log (run-id + effective RNG seed + source path); `axon trace --replay <run-id>` looks up the record and re-executes the source with the same seed → deterministic `(Trace, Seed)` pair for every run (F2 CLI wrapper, ROADMAP §9.5). The run-id is printed to stderr and is the replay handle. Remaining: typed `AuditEvent` emission at capability call sites (F3: Principal-tag + effect-row), `Sandbox<P>` runtime effect enforcement (F5). |
