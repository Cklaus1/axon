...
| F3 | Provenance log no Principal, no effect row, no causal link — **partially closed**: the log already records typed `ai_call` events (fn, tier, routed model, mode live/mock/replay/fallback, metered cost) and **`axon trace --ai` now makes them QUERYABLE** (per-fn audit: calls, tier→model, mode breakdown, total cost; `--json` = stable `axon-ai-audit/1`). The **causal goal→call link** is also done: an `ai_complete` fired inside a `run_goal*` metric evaluation is stamped with the optimizing goal, so the audit reports cost-PER-GOAL (`→ goal \`name\``). Remaining: Principal-tag the events + attach the effect row. | 7 + 9 | Queryable audit + causal goal-link done; Principal/effect-row remain. |
...
| # | Gap | Phase target | Notes |
|---|---|---|---|
| F6 | ~~`@[verify]` predicate language is `confidence OP K` only~~ — **closed for single-Uncertain returns**: the interpreter binds `value` / `confidence` / `source_tag` into a fresh env and evaluates the full predicate Expr at runtime. `@[verify(value >= 50 && confidence >= 0.8)]`, `||` predicates, function calls in the predicate, all work. Relations between two distinct Uncertain values (the F6 "multi-Uncertain" sub-case) still wait on refinement types in Phase 5. | 5 | Done for single-return composite. |
...
| F10 | ~~No reward-shaping syntax~~ — **userland landed**: `examples/stdlib/reward.ax` ships a `Reward { name, score, max }` algebra with `reward_unit/blend/scale/penalize/min/max` combinators. Composes the magic-number expressions buried in @[adaptive] fns into named, normalized, weight-able metrics. Phase-8 `Reward<T>` typed combinator with first-class language support still pending. | 8 | Userland done. |
...
| F12 | ~~No `Agent` type~~ — **userland landed**: `examples/stdlib/agent.ax` ships an `Agent { name, principal, budget_*, strikes, max_strikes, halted, steps_* }` state machine. `agent_step` evaluates the full safety quartet — halted → unauthorized → over-budget → under-confident → under-quality — and returns `(Agent, decision_str)` with the kill-switch latching at `max_strikes`. First-class `Agent<Caps>` with language support waits on Phase 7+8. | 7 + 8 | Userland done. |
