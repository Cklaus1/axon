...
| F3 | Provenance log no Principal, no effect row, no causal link — **partially closed**: the log already records typed `ai_call` events (fn, tier, routed model, mode live/mock/replay/fallback, metered cost) and **`axon trace --ai` now makes them QUERYABLE** (per-fn audit: calls, tier→model, mode breakdown, total cost; `--json` = stable `axon-ai-audit/1`). Remaining: Principal-tag the events, attach the effect row, and the causal goal→call link. | 7 + 9 | The *queryable AI-call audit* half is done; Principal/effect-row/causal-link remain. |
