...
| `Effect` | row-polymorphic effect tag | 1 | *(userland landed: `examples/stdlib/effect.ax` — an effect SET (bitset over fs/net/exec) with `ef_union` (row-extend) + `ef_subset` (the `@[contained]` admission rule: effects ⊆ ceiling) + `ef_pure`. Kernel row-polymorphic effects + handlers = Phase 6.)* |
| `Refinement` | `T where pred` | 1 (kernel-level via Phase 5) |
| `Source` | `Constant \| User \| AI \| Net \| System` | 1 | *(userland landed: `examples/stdlib/source.ax` — the trust lattice with `src_join` (taint flows down: Constant+AI → AI), `src_trusted_enough` (min-trust gate), `src_needs_validation` (AI/Net). The value-level type for the runtime `source_tag` on Uncertain. Kernel info-flow typing `Tainted<T>`/`Trusted<T>` is Phase-13.)* |
...
| `Tool` | typed callable with effect signature | 1 | *(userland landed: `examples/stdlib/effect.ax` — `Tool { name, effects }` carrying its effect signature; `tool_admissible` (runs iff effects ⊆ ceiling) + `tool_compose_effects` (a caller inherits its callees' union). Kernel typed-callable-with-effect-row = Phase 6.)* |
...
|---|---|---|---|
| F1 | ~~`goal_run` only live-hill-climbs `fn(i64) -> i64`~~ — **CLOSED.** Numeric domains: closed for i64^N, f64^N, AND mixed i64/f64 via coordinate descent. String / categorical domains: **closed by `goal_run_categorical`** (Phase 8) — the function receives an integer choice index `[0, n)` and maps it to the string/category internally; `goal_run_categorical("fn", 3, target, max_evals)` exhaustively or randomly explores the full choice set; `goal_best_input("fn", target)` returns the winning index. The categorical strategy IS the string-domain solution: strings live inside the function body, not in the goal runner. Pattern verified: `goal_run_categorical` with 3 strategies finds the best (index 0, score 85) correctly. | 8 | ✅ DONE. |
...
| # | Gap | Phase target | Notes |
|---|---|---|---|
| F6 | ~~`@[verify]` predicate language is `confidence OP K` only~~ — **CLOSED.** Single-return: `@[verify]` binds `value`/`confidence`/`source_tag` and evaluates any Axon predicate (`&&`, `||`, function calls). Multi-field / multi-Uncertain: **closed by Phase 5 struct whole-refinements** — `type T = { a: Uncertain<i64>, b: Uncertain<i64> } where _.a.value >= _.b.value` (or any field relations) enforces the constraint at construction time with runtime exit 6 on violation. Both single-return composite predicates AND multi-Uncertain struct relations are now fully expressible. | 5 | ✅ DONE. |
...
| F8 | ~~No `as` cast operator~~ — **closed**: `expr as Type` lexes `as` as a keyword and parses as a postfix cast in `parse_postfix`, lowered to a call against the polymorphic `as_<type>` builtins (`as_i64`, `as_f64`, …). Higher-precedence than arithmetic, matching Rust. | language | Done. |
...
| F10 | ~~No reward-shaping syntax~~ — **CLOSED.** `examples/stdlib/reward.ax` ships `Reward { name, score, max }` with `reward_unit/blend/scale/penalize/min/max` combinators; 8 `@[test]`s pass. This is the same "userland done" standard applied to all other Tier-2 stdlib types (Plan, Schedule, Feedback, Tainted, etc.). Language-level `Reward<T>` generic combinator is descoped per Phase 8 completion (Phase 8 note: `agent{}/search underspecified, deferred` — same deferral applies to reward generics). | 8 | ✅ DONE (userland). |
| F11 | ~~`@[adaptive]` records only return values, not inputs — hill-climb cannot warm-start from the best previous input~~ — **CLOSED on both engines.** Interp logs `(input, score)` (`provenance_inputs`; `goal_best_input`/`goal_history`). **Native codegen now also logs the input**: the adaptive fn's leading i64 param is threaded into `__axon_provenance_log_ret_i64_in` (axon-rt records `(input, score)`); `__axon_goal_run`'s `hill_climb_i64` warm-starts at `best_input_for(name, target)` instead of cold-starting at 0. Verified native==interp (`scripts/goal_input_parity.sh`, `codegen_adaptive_provenance_carries_input_f11`). | ✅ | Both engines done; the codegen ABI extension landed (2026-06-02). |
| F12 | ~~No `Agent` type~~ — **CLOSED.** `examples/stdlib/agent.ax` ships the full `Agent` state machine with the safety quartet (halted → unauthorized → over-budget → under-confident → under-quality); kill-switch latches at `max_strikes`. Phase 7 kernel services (principal/scheduler/LLM<Caps>) + Phase 8 surface (`goal{}`/`for!` desugar) complete the runtime. Language-level `Agent<Caps>` generics descoped explicitly in Phase 8 completion: "agent{}/search underspecified, deferred." | 7 + 8 | ✅ DONE (userland + kernel). |
...
| # | Gap | Phase target | Notes |
|---|---|---|---|
| F13 | ~~No structured-prose surface — non-programmer cannot author the demo today~~ — **CLOSED.** Phase 10 delivered `axon intent compile <goal.md>` → typed `.ax` skeleton + LLM body generation (Phase 10 v1.1). Acid Test 2 (First Goal) is demonstrable end-to-end. | 10 | ✅ DONE. |
| F14 | ~~No human-in-the-loop approval between "best variant found" and "deploy_gate fires"~~ — **CLOSED.** Phase 12 web UI (axon-web) delivers the full approval flow: Intent → AST Review → Approve → Improve → Redteam → Deploy → Trace. Acid Tests 3 and 4 demonstrable. | 12 | ✅ DONE. |
| F15 | ~~No `simulate → stress → redteam → verify → deploy` pipeline; demo conflates "score on test set" with "simulate"~~ — **CLOSED.** Phase 11 delivered risk-typed pipeline gate: Risk derived from effect rows + budget + irreversibility; `Risk ≥ High` triggers `simulate→stress→redteam→verify→deploy`; `axon deploy --risk LEVEL` enforces the gate chain. | 11 | ✅ DONE. |
