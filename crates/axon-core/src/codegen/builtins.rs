...
impl<'ctx> super::Codegen<'ctx> {
    ...
    pub fn declare_builtins(&mut self) {
        ...
        let strncmp_ty0 = i32_ty.fn_type(
            &[i8_ptr.into(), i8_ptr.into(), malloc_size_ty.into()],
            false,
        );
        ...
        // axon_println: takes { i64, i8* } Axon str struct, calls puts on the data ptr
        {
            ...
            let data_ptr =
                build_wrappers::w_extract_value(&self.ir.builder, str_arg, 1, "data_ptr")
                    .into_pointer_value();
        ...
        let write_ty = write_ret_ty.fn_type(
            &[i32_ty.into(), i8_ptr.into(), malloc_size_ty.into()],
            false,
        );
        ...
        // eprintln: writes string + newline to stderr (fd=2) using write(2, ...).
        {
            ...
            let nl_ptr = build_wrappers::w_pointer_cast(
                &self.ir.builder,
            ...
            build_wrappers::w_call(
                &self.ir.builder,
                write_fn,
                &[fd2.into(), nl_ptr.into(), self.msize(one64, "msz").into()],
                "",
            );
            build_wrappers::w_ret_void(&self.ir.builder);
            if let Some(b) = saved_block {
        ...
        // in this loop. The interpreter is the reference semantics.
        {
            ...
            let rt_fn = self
                .ir
                .module
                .get_function("__axon_i64_to_str_radix")
                .unwrap_or_else(|| {
                    self.ir
                        .module
                        .add_function("__axon_i64_to_str_radix", rt_ty, None)
                });
            ...
            let out_ptr_cast = build_wrappers::w_pointer_cast(
                &self.ir.builder,
                out_ptr,
                i8_ptr_ptr,
                "ts_optrcast",
            ...
            build_wrappers::w_call(
                ...
                &[
                    n.into(),
                    i64_ty.const_int(10, false).into(),
                    out_len.into(),
                    out_ptr_cast.into(),
                ],
                "ts_call",
            );
            let len = build_wrappers::w_load(&self.ir.builder, i64_ty.into(), out_len, "ts_len")
                .into_int_value();
            let ptr = build_wrappers::w_load(&self.ir.builder, i8_ptr.into(), out_ptr, "ts_ptr")
                .into_pointer_value();
            let mut result = str_ty.const_zero();
            result =
                build_wrappers::w_insert_value(&self.ir.builder, result, len.into(), 0, "ts_r0")
                    .into_struct_value();
            result =
                build_wrappers::w_insert_value(&self.ir.builder, result, ptr.into(), 1, "ts_r1")
                    .into_struct_value();
        ...
        // printing links on the browser target (wasm32-unknown-unknown) too.
        {
            ...
            let rt_ty = self
                ...
                .context
                .void_type()
                .fn_type(&[f64_ty.into(), i64_ptr.into(), i8_ptr_ptr.into()], false);
            let rt_fn = self
                .ir
                .module
                .get_function("__axon_f64_to_str")
                .unwrap_or_else(|| {
                    self.ir
                        .module
                        .add_function("__axon_f64_to_str", rt_ty, None)
                });
            ...
            build_wrappers::w_call(
                &self.ir.builder,
                rt_fn,
                &[x.into(), out_len.into(), out_ptr_cast.into()],
                "tf_call",
            );
            let len = build_wrappers::w_load(&self.ir.builder, i64_ty.into(), out_len, "tf_len")
                .into_int_value();
            let ptr = build_wrappers::w_load(&self.ir.builder, i8_ptr.into(), out_ptr, "tf_ptr")
                .into_pointer_value();
            let mut result = str_ty.const_zero();
            result =
                build_wrappers::w_insert_value(&self.ir.builder, result, len.into(), 0, "tf_r0")
                    .into_struct_value();
            result =
                build_wrappers::w_insert_value(&self.ir.builder, result, ptr.into(), 1, "tf_r1")
                    .into_struct_value();
            build_wrappers::w_ret(&self.ir.builder, result.into());
            if let Some(b) = saved {
                self.ir.builder.position_at_end(b);
        ...
        // assert_eq(a: i64, b: i64): panic if a != b
        {
            ...
            let eq =
                build_wrappers::w_int_compare(&self.ir.builder, IntPredicate::EQ, a, b_param, "eq");
            ...
            let p = self
                .ir
                .module
                .get_function("__axon_assert_eq_i64_panic")
                .unwrap_or_else(|| {
                    self.ir
                        .module
                        .add_function("__axon_assert_eq_i64_panic", aei_ty, None)
                });
        ...
        // assert_err(tag: i1): panic if tag == 1 (Ok) — expected Err
        {
            ...
            let is_ok_val = build_wrappers::w_int_compare(
                &self.ir.builder,
                IntPredicate::EQ,
                tag,
        ...
        // len(s: str) -> i64: extracts the length field (field 0) from the str struct
        {
            ...
            let length =
                build_wrappers::w_extract_value(&self.ir.builder, s, 0, "len").into_int_value();
        ...
        // parse_int byte-for-byte. (`parse_int_err_parity.sh` guards this.)
        {
            ...
            let result_ty = self
                .ir
                .context
                .struct_type(&[bool_ty.into(), i8_arr16_ty.into()], false);
            let str_ty = self
                .ir
                .context
                .struct_type(&[i64_ty.into(), i8_ptr.into()], false);
            ...
            let rt_fn = self
                .ir
                .module
                .get_function("__axon_parse_int_radix")
                .unwrap_or_else(|| {
                    self.ir
                        .module
                        .add_function("__axon_parse_int_radix", rt_ty, None)
                });
            let out_ok_slot =
                build_wrappers::w_alloca(&self.ir.builder, i64_ty.into(), "pi_ok_slot");
            let out_val_slot =
                build_wrappers::w_alloca(&self.ir.builder, i64_ty.into(), "pi_val_slot");
            let out_len_slot =
                build_wrappers::w_alloca(&self.ir.builder, i64_ty.into(), "pi_len_slot");
            let out_ptr_slot =
                build_wrappers::w_alloca(&self.ir.builder, i8_ptr.into(), "pi_ptr_slot");
            ...
            let ok_flag =
                build_wrappers::w_load(&self.ir.builder, i64_ty.into(), out_ok_slot, "pi_okflag")
                    .into_int_value();
            let zero = i64_ty.const_int(0, false);
            let is_ok = build_wrappers::w_int_compare(
                &self.ir.builder,
                IntPredicate::NE,
                ok_flag,
                zero,
            ...
            self.ir.builder.position_at_end(ok_bb);
            let parsed =
                build_wrappers::w_load(&self.ir.builder, i64_ty.into(), out_val_slot, "pi_parsed")
                    .into_int_value();
            let ok_alloca =
                build_wrappers::w_alloca(&self.ir.builder, result_ty.into(), "pi_ok_alloca");
            let tag1 = bool_ty.const_int(1, false);
            let tag_ptr_ok = build_wrappers::w_struct_gep(
                &self.ir.builder,
                result_ty.into(),
                ok_alloca,
                0,
            ...
            let payload_ptr_ok = build_wrappers::w_struct_gep(
                &self.ir.builder,
                result_ty.into(),
                ok_alloca,
                1,
            ...
            let payload_i64_ptr = build_wrappers::w_pointer_cast(
                &self.ir.builder,
                payload_ptr_ok,
                i64_ptr,
                "pi_payi64",
            ...
            let ok_val =
                build_wrappers::w_load(&self.ir.builder, result_ty.into(), ok_alloca, "pi_okval");
            ...
            let perr_ty =
                void_ty.fn_type(&[str_ty.into(), i64_ptr.into(), i8_ptr_ptr.into()], false);
            let perr_fn = self
                .ir
                .module
                .get_function("__axon_parse_int_err")
                .unwrap_or_else(|| {
                    self.ir
                        .module
                        .add_function("__axon_parse_int_err", perr_ty, None)
                });
            build_wrappers::w_call(
                &self.ir.builder,
                perr_fn,
            ...
            let err_str_ty = self
                ...
                .struct_type(&[i64_ty.into(), i8_ptr.into()], false);
            let msg_len =
                build_wrappers::w_load(&self.ir.builder, i64_ty.into(), out_len_slot, "pi_emlen")
                    .into_int_value();
            let msg_ptr =
                build_wrappers::w_load(&self.ir.builder, i8_ptr.into(), out_ptr_slot, "pi_emptr")
                    .into_pointer_value();
            let err_alloca =
                build_wrappers::w_alloca(&self.ir.builder, result_ty.into(), "pi_err_alloca");
            let tag0 = bool_ty.const_int(0, false);
            let tag_ptr_err = build_wrappers::w_struct_gep(
                &self.ir.builder,
                result_ty.into(),
                err_alloca,
                0,
            ...
            let payload_ptr_err = build_wrappers::w_struct_gep(
                &self.ir.builder,
                result_ty.into(),
                err_alloca,
                1,
            ...
            let payload_str_ptr = build_wrappers::w_pointer_cast(
                &self.ir.builder,
                payload_ptr_err,
                err_str_ty.ptr_type(inkwell::AddressSpace::default()),
                "pi_paystr",
            );
            let err_str_alloca =
                build_wrappers::w_alloca(&self.ir.builder, err_str_ty.into(), "pi_errstr");
            let esl = build_wrappers::w_struct_gep(
                &self.ir.builder,
                err_str_ty.into(),
                err_str_alloca,
                0,
            ...
            let esd = build_wrappers::w_struct_gep(
                &self.ir.builder,
                err_str_ty.into(),
                err_str_alloca,
                1,
            ...
            let err_str_val = build_wrappers::w_load(
                &self.ir.builder,
                err_str_ty.into(),
                err_str_alloca,
                "pi_errstrval",
            ...
            let err_val =
                build_wrappers::w_load(&self.ir.builder, result_ty.into(), err_alloca, "pi_errval");
        ...
        // from the out-params. Byte-identical to the interpreter by construction.
        {
            ...
            let out_ok_slot =
                build_wrappers::w_alloca(&self.ir.builder, i64_ty.into(), "pir_ok_slot");
            let out_val_slot =
                build_wrappers::w_alloca(&self.ir.builder, i64_ty.into(), "pir_val_slot");
            let out_len_slot =
                build_wrappers::w_alloca(&self.ir.builder, i64_ty.into(), "pir_len_slot");
            let out_ptr_slot =
                build_wrappers::w_alloca(&self.ir.builder, i8_ptr.into(), "pir_ptr_slot");
            let out_ptr_slot_cast = build_wrappers::w_pointer_cast(
                &self.ir.builder,
                out_ptr_slot,
                i8_ptr_ptr,
            ...
            let ok_flag =
                build_wrappers::w_load(&self.ir.builder, i64_ty.into(), out_ok_slot, "pir_okflag")
                    .into_int_value();
            let zero = i64_ty.const_int(0, false);
            let is_ok = build_wrappers::w_int_compare(
                &self.ir.builder,
            ...
            self.ir.builder.position_at_end(ok_bb);
            let parsed =
                build_wrappers::w_load(&self.ir.builder, i64_ty.into(), out_val_slot, "pir_parsed")
                    .into_int_value();
            let ok_alloca =
                build_wrappers::w_alloca(&self.ir.builder, result_ty.into(), "pir_ok_alloca");
            let tag1 = bool_ty.const_int(1, false);
            let tag_ptr_ok = build_wrappers::w_struct_gep(
                &self.ir.builder,
                result_ty.into(),
            ...
            build_wrappers::w_store(&self.ir.builder, payload_i64_ptr, parsed.into());
            let ok_val =
                build_wrappers::w_load(&self.ir.builder, result_ty.into(), ok_alloca, "pir_okval");
            build_wrappers::w_ret(&self.ir.builder, ok_val);
            ...
            let err_str_ty = self
                ...
                .struct_type(&[i64_ty.into(), i8_ptr.into()], false);
            let msg_len =
                build_wrappers::w_load(&self.ir.builder, i64_ty.into(), out_len_slot, "pir_emlen")
                    .into_int_value();
            let msg_ptr =
                build_wrappers::w_load(&self.ir.builder, i8_ptr.into(), out_ptr_slot, "pir_emptr")
                    .into_pointer_value();
            let err_alloca =
                build_wrappers::w_alloca(&self.ir.builder, result_ty.into(), "pir_err_alloca");
            let tag0 = bool_ty.const_int(0, false);
            let tag_ptr_err = build_wrappers::w_struct_gep(
                &self.ir.builder,
                result_ty.into(),
            ...
            let err_str_alloca =
                build_wrappers::w_alloca(&self.ir.builder, err_str_ty.into(), "pir_errstr");
            let esl = build_wrappers::w_struct_gep(
                &self.ir.builder,
                err_str_ty.into(),
                err_str_alloca,
            ...
            build_wrappers::w_store(&self.ir.builder, payload_str_ptr, err_str_val);
            let err_val = build_wrappers::w_load(
                &self.ir.builder,
                result_ty.into(),
                err_alloca,
                "pir_errval",
        ...
        // Allocates a new buffer via malloc, copies both strings, null-terminates.
        {
            ...
            let malloc_fn = self
                ...
                .module
                .get_function("malloc")
                .unwrap_or_else(|| self.ir.module.add_function("malloc", malloc_ty, None));
            ...
            let memcpy_fn = self
                ...
                .module
                .get_function("memcpy")
                .unwrap_or_else(|| self.ir.module.add_function("memcpy", memcpy_ty, None));
            ...
            let a_len = build_wrappers::w_extract_value(&self.ir.builder, a_val, 0, "a_len")
                .into_int_value();
            let a_ptr = build_wrappers::w_extract_value(&self.ir.builder, a_val, 1, "a_ptr")
                .into_pointer_value();
            let b_len = build_wrappers::w_extract_value(&self.ir.builder, b_val, 0, "b_len")
                .into_int_value();
            let b_ptr = build_wrappers::w_extract_value(&self.ir.builder, b_val, 1, "b_ptr")
                .into_pointer_value();
            ...
            let one64 = i64_ty.const_int(1, false);
            let alloc_len =
                build_wrappers::w_int_add(&self.ir.builder, total_len, one64, "alloc_len");

            // buf = malloc(alloc_len)
            let buf = build_wrappers::w_call(
                &self.ir.builder,
                malloc_fn,
                &[self.msize(alloc_len, "msz").into()],
                "buf",
            );
            let buf_ptr = buf
                .try_as_basic_value()
            ...
            build_wrappers::w_store(
                &self.ir.builder,
                null_pos,
                self.ir.context.i8_type().const_int(0, false).into(),
            );

            // Return { total_len, buf_ptr }
            let out_alloca =
                build_wrappers::w_alloca(&self.ir.builder, str_ty.into(), "concat_out");
            let len_ptr = build_wrappers::w_struct_gep(
                &self.ir.builder,
                str_ty.into(),
                out_alloca,
                0,
            ...
            let dat_ptr = build_wrappers::w_struct_gep(
                &self.ir.builder,
                str_ty.into(),
                out_alloca,
                1,
            ...
            let out =
                build_wrappers::w_load(&self.ir.builder, str_ty.into(), out_alloca, "concat_val");
        ...
        let chan_new_fn = self
            .ir
            .module
            .add_function("__axon_chan_new", chan_new_ty, None);
        self.functions
            .insert("__axon_chan_new".to_string(), chan_new_fn);

        // __axon_chan_send: void __axon_chan_send(chan: i8*, val: i64)
        let chan_send_ty = void_ty.fn_type(&[i8_ptr.into(), i64_ty.into()], false);
        let chan_send_fn = self
            .ir
            .module
            .add_function("__axon_chan_send", chan_send_ty, None);
        self.functions
            .insert("__axon_chan_send".to_string(), chan_send_fn);

        // __axon_chan_recv: i64 __axon_chan_recv(chan: i8*)
        let chan_recv_ty = i64_ty.fn_type(&[i8_ptr.into()], false);
        let chan_recv_fn = self
            .ir
            .module
            .add_function("__axon_chan_recv", chan_recv_ty, None);
        self.functions
            .insert("__axon_chan_recv".to_string(), chan_recv_fn);

        // __axon_select: i64 __axon_select(chans: i8**, n: i64)
        // Returns the index of the first ready channel arm.
        let select_ty = i64_ty.fn_type(
            &[
        ...
        let select_fn = self
            .ir
            .module
            .add_function("__axon_select", select_ty, None);
        self.functions
            .insert("__axon_select".to_string(), select_fn);

        // __axon_chan_clone: i8* __axon_chan_clone(chan: i8*)
        let chan_clone_ty = i8_ptr.fn_type(&[i8_ptr.into()], false);
        let chan_clone_fn = self
            .ir
            .module
            .add_function("__axon_chan_clone", chan_clone_ty, None);
        self.functions
            .insert("__axon_chan_clone".to_string(), chan_clone_fn);
        ...
        self.fn_return_types
            .insert("Chan::new".to_string(), Type::Chan(Box::new(Type::Unknown)));
        self.fn_return_types.insert("recv".to_string(), Type::I64);
        self.fn_return_types.insert("send".to_string(), Type::Unit);
        self.fn_return_types
            .insert("clone".to_string(), Type::Chan(Box::new(Type::Unknown)));
        ...
        self.ir
            .module
            .add_function("__axon_dict_remove", dr_ty, None);
        // void __axon_dict_keys(d:i8*, out_len:i64*, out_data:i8**)
        let i8_ptr_ptr_d = i8_ptr.ptr_type(inkwell::AddressSpace::default());
        let dk_ty = void_ty.fn_type(
            &[i8_ptr.into(), i64_ptr_d.into(), i8_ptr_ptr_d.into()],
            false,
        );
        ...
        let dvl_ty = void_ty.fn_type(
            &[i8_ptr.into(), i64_ptr_d.into(), i8_ptr_ptr_d.into()],
            false,
        );
        self.ir
            .module
            .add_function("__axon_dict_values", dvl_ty, None);
        ...
        self.ir
            .module
            .add_function("__axon_str_split", ssplit_ty, None);
        ...
        self.ir
            .module
            .add_function("__axon_dict_from_pairs", dfp_ty, None);
        self.fn_return_types.insert(
            "dict_from_pairs".to_string(),
            Type::Deferred("Dict".to_string()),
        );
        ...
        let dtp_ty = void_ty.fn_type(
            &[i8_ptr.into(), i64_ptr_d.into(), i8_ptr_ptr_d.into()],
            false,
        );
        self.ir
            .module
            .add_function("__axon_dict_to_pairs", dtp_ty, None);
        ...
        self.ir
            .module
            .add_function("__axon_dict_map_values", dmv_ty, None);
        self.fn_return_types.insert(
            "dict_map_values".to_string(),
            Type::Deferred("Dict".to_string()),
        );
        ...
        self.ir
            .module
            .add_function("__axon_dict_filter", dfl_ty, None);
        self.fn_return_types.insert(
            "dict_filter".to_string(),
            Type::Deferred("Dict".to_string()),
        );
        ...
        self.ir
            .module
            .add_function("__axon_dict_each", dea_ty, None);
        self.fn_return_types
            .insert("dict_each".to_string(), Type::Unit);
        ...
        self.fn_return_types
            .insert("dict_set".to_string(), Type::Unit);

        // Populate fn_return_types for all other builtins (Fix 19).
        self.fn_return_types
            .insert("println".to_string(), Type::Unit);
        self.fn_return_types.insert("print".to_string(), Type::Unit);
        self.fn_return_types
            .insert("eprintln".to_string(), Type::Unit);
        self.fn_return_types
            .insert("eprint".to_string(), Type::Unit);
        self.fn_return_types
            .insert("assert".to_string(), Type::Unit);
        self.fn_return_types
            .insert("assert_eq".to_string(), Type::Unit);
        self.fn_return_types
            .insert("assert_err".to_string(), Type::Unit);
        self.fn_return_types.insert("len".to_string(), Type::I64);
        self.fn_return_types.insert("to_str".to_string(), Type::Str);
        self.fn_return_types
            .insert("to_str_f64".to_string(), Type::Str);
        self.fn_return_types
            .insert("axon_concat".to_string(), Type::Str);
        ...
        // assert_eq_f64 — panic if two f64 values differ.
        {
            ...
            let eq = build_wrappers::w_float_compare(
                &self.ir.builder,
                FloatPredicate::OEQ,
                a,
                b_param,
            ...
            let p = self
                .ir
                .module
                .get_function("__axon_assert_eq_f64_panic")
                .unwrap_or_else(|| {
                    self.ir
                        .module
                        .add_function("__axon_assert_eq_f64_panic", aef_ty, None)
                });
            build_wrappers::w_call(&self.ir.builder, p, &[a.into(), b_param.into()], "");
            build_wrappers::w_unreachable(&self.ir.builder);
            self.ir.builder.position_at_end(ok_bb);
            ...
            self.fn_return_types
                .insert("assert_eq_f64".to_string(), Type::Unit);
        }
        ...
        // assert_eq_str — panic if two str values differ (compare len then bytes via memcmp).
        {
            ...
            let a_len = build_wrappers::w_extract_value(&self.ir.builder, a_struct, 0, "a_len")
                .into_int_value();
            let b_len = build_wrappers::w_extract_value(&self.ir.builder, b_struct, 0, "b_len")
                .into_int_value();
            let len_eq = build_wrappers::w_int_compare(
                &self.ir.builder,
                IntPredicate::EQ,
                a_len,
                b_len,
            ...
            let p = self
                .ir
                .module
                .get_function("__axon_assert_eq_str_panic")
                .unwrap_or_else(|| {
                    self.ir
                        .module
                        .add_function("__axon_assert_eq_str_panic", aes_ty, None)
                });
            ...
            let a_ptr = build_wrappers::w_extract_value(&self.ir.builder, a_struct, 1, "a_ptr")
                .into_pointer_value();
            let b_ptr = build_wrappers::w_extract_value(&self.ir.builder, b_struct, 1, "b_ptr")
                .into_pointer_value();
            ...
            let memcmp_fn = self
                ...
                .module
                .get_function("memcmp")
                .unwrap_or_else(|| self.ir.module.add_function("memcmp", memcmp_ty, None));
            let cmp_result = build_wrappers::w_call(
                &self.ir.builder,
                memcmp_fn,
                &[a_ptr.into(), b_ptr.into(), a_len.into()],
                "cmp",
            )
            .try_as_basic_value()
            .left()
            ...
            self.fn_return_types
                .insert("assert_eq_str".to_string(), Type::Unit);
        }
        ...
        //                      f64 decay, i64 valid_until_ms }
        {
            ...
            // uncertain_new(value: i64, confidence: f64) -> Uncertain<i64>
            {
                ...
                sv = build_wrappers::w_insert_value(&self.ir.builder, sv, v.into(), 0, "u_val")
                    .into_struct_value();
                sv = build_wrappers::w_insert_value(&self.ir.builder, sv, c.into(), 1, "u_conf")
                    .into_struct_value();
            ...
            // uncertain_deterministic(value: i64) -> Uncertain<i64> (confidence = 1.0)
            {
                ...
                let fn_val = self
                    .ir
                    .module
                    .add_function("uncertain_deterministic", fn_ty, None);
                let bb = self.ir.context.append_basic_block(fn_val, "entry");
                self.ir.builder.position_at_end(bb);
                let v = fn_val.get_nth_param(0).unwrap().into_int_value();
                let one = f64_ty.const_float(1.0);
                let mut sv = unc_ty.get_undef();
                sv = build_wrappers::w_insert_value(&self.ir.builder, sv, v.into(), 0, "ud_val")
                    .into_struct_value();
                sv = build_wrappers::w_insert_value(&self.ir.builder, sv, one.into(), 1, "ud_conf")
                    .into_struct_value();
                sv = build_wrappers::w_insert_value(
                    &self.ir.builder,
                    sv,
                ...
                build_wrappers::w_ret(&self.ir.builder, sv.into());
                self.functions
                    .insert("uncertain_deterministic".to_string(), fn_val);
                self.fn_return_types.insert(
            ...
            // value slot.
            {
                ...
                let fn_val = self
                    .ir
                    .module
                    .add_function("uncertain_new_f64", fn_ty, None);
                let bb = self.ir.context.append_basic_block(fn_val, "entry");
                self.ir.builder.position_at_end(bb);
                let v = fn_val.get_nth_param(0).unwrap().into_float_value();
                let c = fn_val.get_nth_param(1).unwrap().into_float_value();
                let mut sv = unc_f64_ty.get_undef();
                sv = build_wrappers::w_insert_value(&self.ir.builder, sv, v.into(), 0, "uf_val")
                    .into_struct_value();
                sv = build_wrappers::w_insert_value(&self.ir.builder, sv, c.into(), 1, "uf_conf")
                    .into_struct_value();
                sv = build_wrappers::w_insert_value(
                    &self.ir.builder,
                    sv,
                ...
                build_wrappers::w_ret(&self.ir.builder, sv.into());
                self.functions
                    .insert("uncertain_new_f64".to_string(), fn_val);
                self.fn_return_types.insert(
                    "uncertain_new_f64".to_string(),
                    Type::Uncertain(Box::new(Type::F64)),
                );
            }
            ...
            // injected by `emit_verify_check_if_needed` at every return site).
            {
                ...
                let fn_val = self
                    .ir
                    .module
                    .add_function("uncertain_dyn_i64", fn_ty, None);
                let bb = self.ir.context.append_basic_block(fn_val, "entry");
                self.ir.builder.position_at_end(bb);
                let v = fn_val.get_nth_param(0).unwrap().into_int_value();
                let c = fn_val.get_nth_param(1).unwrap().into_float_value();
                let mut sv = unc_ty.get_undef();
                sv = build_wrappers::w_insert_value(&self.ir.builder, sv, v.into(), 0, "udy_val")
                    .into_struct_value();
                sv = build_wrappers::w_insert_value(&self.ir.builder, sv, c.into(), 1, "udy_conf")
                    .into_struct_value();
                sv = build_wrappers::w_insert_value(
                    &self.ir.builder,
                    sv,
                    i64_ty.const_int(2, false).into(),
                    2,
                    "udy_src",
                )
                .into_struct_value();
                build_wrappers::w_ret(&self.ir.builder, sv.into());
                self.functions
                    .insert("uncertain_dyn_i64".to_string(), fn_val);
                self.fn_return_types.insert(
                    "uncertain_dyn_i64".to_string(),
                    Type::Uncertain(Box::new(Type::I64)),
                );
            }
            ...
            // f64 variant of uncertain_dyn_i64.  source_tag = 2 (Runtime).
            {
                ...
                let fn_val = self
                    .ir
                    .module
                    .add_function("uncertain_dyn_f64", fn_ty, None);
                let bb = self.ir.context.append_basic_block(fn_val, "entry");
                self.ir.builder.position_at_end(bb);
                let v = fn_val.get_nth_param(0).unwrap().into_float_value();
                let c = fn_val.get_nth_param(1).unwrap().into_float_value();
                let mut sv = unc_f64_ty.get_undef();
                sv = build_wrappers::w_insert_value(&self.ir.builder, sv, v.into(), 0, "udyf_val")
                    .into_struct_value();
                sv = build_wrappers::w_insert_value(&self.ir.builder, sv, c.into(), 1, "udyf_conf")
                    .into_struct_value();
                sv = build_wrappers::w_insert_value(
                    &self.ir.builder,
                    sv,
                ...
                build_wrappers::w_ret(&self.ir.builder, sv.into());
                self.functions
                    .insert("uncertain_dyn_f64".to_string(), fn_val);
                self.fn_return_types.insert(
                    "uncertain_dyn_f64".to_string(),
                    Type::Uncertain(Box::new(Type::F64)),
                );
            }
            ...
            // valid_until_ms = __axon_now_ms() + horizon_ms; confidence starts at 1.0.
            {
                ...
                let now = build_wrappers::w_call(&self.ir.builder, now_fn, &[], "tn_now")
                    ...
                    .into_int_value();
                let valid_until =
                    build_wrappers::w_int_add(&self.ir.builder, now, horizon, "tn_valid_until");
                let one = f64_ty.const_float(1.0);
                let mut sv = tmp_ty.get_undef();
                sv = build_wrappers::w_insert_value(&self.ir.builder, sv, v.into(), 0, "tn_val")
                    .into_struct_value();
                sv = build_wrappers::w_insert_value(&self.ir.builder, sv, one.into(), 1, "tn_conf")
                    .into_struct_value();
                sv = build_wrappers::w_insert_value(
                    &self.ir.builder,
                    sv,
                ...
                self.fn_return_types.insert(
                    "temporal_new".to_string(),
                    Type::Temporal(Box::new(Type::I64)),
                );
            }
            ...
            // confidence unchanged. valid_until_ms is shifted by offset_ms.
            {
                ...
                let offset_f = build_wrappers::w_signed_int_to_float(
                    &self.ir.builder,
                    offset_ms,
                    f64_ty,
                    "ta_offf",
                ...
                let days =
                    build_wrappers::w_float_div(&self.ir.builder, offset_f, day_ms, "ta_days");
                let one = f64_ty.const_float(1.0);
                let zero = f64_ty.const_float(0.0);
                // base = max(0, 1 - decay); decayed = conf * base^days.
                let one_minus_decay =
                    build_wrappers::w_float_sub(&self.ir.builder, one, decay, "ta_1md");
                let base_neg = build_wrappers::w_float_compare(
                    &self.ir.builder,
                ...
                let base = build_wrappers::w_select(
                    &self.ir.builder,
                    base_neg,
                    zero.into(),
                    one_minus_decay.into(),
                    "ta_base",
                )
                .into_float_value();
                let powed = build_wrappers::w_call(
                    &self.ir.builder,
                    pow_fn,
                    &[base.into(), days.into()],
                    "ta_pow",
                ...
                .into_float_value();
                let decayed =
                    build_wrappers::w_float_mul(&self.ir.builder, conf, powed, "ta_decayed");
                // Only decay for a positive offset; otherwise keep `conf`.
                let pos = build_wrappers::w_float_compare(
                    &self.ir.builder,
                ...
                let new_conf = build_wrappers::w_select(
                    &self.ir.builder,
                    pos,
                    decayed.into(),
                    conf.into(),
                ...
                .into_float_value();
                let new_valid =
                    build_wrappers::w_int_add(&self.ir.builder, valid_until, offset_ms, "ta_nvu");
            ...
            // Returns __axon_now_ms() <= valid_until_ms.
            {
                ...
                let fn_val = self
                    .ir
                    .module
                    .add_function("temporal_is_valid", fn_ty, None);
                let bb = self.ir.context.append_basic_block(fn_val, "entry");
                self.ir.builder.position_at_end(bb);
                let t = fn_val.get_nth_param(0).unwrap().into_struct_value();
                ...
                self.functions
                    .insert("temporal_is_valid".to_string(), fn_val);
                self.fn_return_types
            ...
            // temporal_confidence(t: Temporal<i64>) -> f64 — extract field 1.
            {
                ...
                let fn_val = self
                    .ir
                    .module
                    .add_function("temporal_confidence", fn_ty, None);
                let bb = self.ir.context.append_basic_block(fn_val, "entry");
                self.ir.builder.position_at_end(bb);
                let t = fn_val.get_nth_param(0).unwrap().into_struct_value();
                ...
                self.functions
                    .insert("temporal_confidence".to_string(), fn_val);
                self.fn_return_types
                    .insert("temporal_confidence".to_string(), Type::F64);
            }
            ...
            if self
                .ir
                .module
                .get_function("uncertain_confidence")
                .is_none()
            {
                ...
                let fn_val = self
                    .ir
                    .module
                    .add_function("uncertain_confidence", fn_ty, None);
                let bb = self.ir.context.append_basic_block(fn_val, "entry");
                self.ir.builder.position_at_end(bb);
                build_wrappers::w_ret_void(&self.ir.builder);
                self.functions
                    .insert("uncertain_confidence".to_string(), fn_val);
                self.fn_return_types
                    .insert("uncertain_confidence".to_string(), Type::Unit);
            }
            ...
            if self.ir.module.get_function("temporal_now").is_none() {
                ...
                self.fn_return_types
                    .insert("temporal_now".to_string(), Type::I64);
            }
        ...
        // packages the result into the Axon `{ i64, i8* }` str struct.
        {
            ...
            let ptr_slot_cast =
                build_wrappers::w_pointer_cast(&self.ir.builder, ptr_slot, i8_ptr_ptr, "ptrptr");
            build_wrappers::w_call(
                &self.ir.builder,
                rt_fn,
                &[len_slot.into(), ptr_slot_cast.into()],
                "",
            );

            let len_val = build_wrappers::w_load(&self.ir.builder, i64_ty.into(), len_slot, "len")
                .into_int_value();
            let ptr_val = build_wrappers::w_load(&self.ir.builder, i8_ptr.into(), ptr_slot, "ptr")
                .into_pointer_value();

            let mut result = str_ty.const_zero();
            result =
                build_wrappers::w_insert_value(&self.ir.builder, result, len_val.into(), 0, "str0")
                    .into_struct_value();
            result =
                build_wrappers::w_insert_value(&self.ir.builder, result, ptr_val.into(), 1, "str1")
                    .into_struct_value();
            build_wrappers::w_ret(&self.ir.builder, result.into());
            ...
            self.fn_return_types
                .insert("read_line".to_string(), Type::Str);
        }
        ...
        // tag=1 → Ok; payload = str{len, ptr}. tag=0 → Err; payload = str{|len|, ptr}.
        {
            ...
            let path_len =
                build_wrappers::w_extract_value(&self.ir.builder, path_str, 0, "rf_plen")
                    .into_int_value();
            let path_ptr_v =
                build_wrappers::w_extract_value(&self.ir.builder, path_str, 1, "rf_pptr")
                    .into_pointer_value();

            let out_len_slot =
                build_wrappers::w_alloca(&self.ir.builder, i64_ty.into(), "rf_out_len");
            let out_ptr_slot =
                build_wrappers::w_alloca(&self.ir.builder, i8_ptr.into(), "rf_out_ptr");
            let out_ptr_cast = build_wrappers::w_pointer_cast(
                &self.ir.builder,
                out_ptr_slot,
                i8_ptr_ptr,
            ...
            let out_len =
                build_wrappers::w_load(&self.ir.builder, i64_ty.into(), out_len_slot, "rf_len")
                    .into_int_value();
            let out_ptr =
                build_wrappers::w_load(&self.ir.builder, i8_ptr.into(), out_ptr_slot, "rf_ptr")
                    .into_pointer_value();
            ...
            self.ir.builder.position_at_end(ok_bb);
            let ok_alloca =
                build_wrappers::w_alloca(&self.ir.builder, result_ty.into(), "rf_ok_slot");
            let tag_ptr_ok = build_wrappers::w_struct_gep(
                &self.ir.builder,
                result_ty.into(),
                ok_alloca,
            ...
            let payload_ok = build_wrappers::w_struct_gep(
                &self.ir.builder,
                result_ty.into(),
                ok_alloca,
                1,
            ...
            let str_ok_ptr = build_wrappers::w_pointer_cast(
                &self.ir.builder,
                payload_ok,
                str_ty.ptr_type(inkwell::AddressSpace::default()),
                "rf_str_ok_ptr",
            );
            let str_ok_slot =
                build_wrappers::w_alloca(&self.ir.builder, str_ty.into(), "rf_str_ok");
            build_wrappers::w_store(
                &self.ir.builder,
                build_wrappers::w_struct_gep(&self.ir.builder, str_ty.into(), str_ok_slot, 0, ""),
                out_len.into(),
            );
            build_wrappers::w_store(
                &self.ir.builder,
                build_wrappers::w_struct_gep(&self.ir.builder, str_ty.into(), str_ok_slot, 1, ""),
                out_ptr.into(),
            );
            let str_ok_val = build_wrappers::w_load(
                &self.ir.builder,
                str_ty.into(),
                str_ok_slot,
                "rf_str_ok_val",
            ...
            let ok_val =
                build_wrappers::w_load(&self.ir.builder, result_ty.into(), ok_alloca, "rf_ok_val");
            build_wrappers::w_ret(&self.ir.builder, ok_val);
            ...
            let err_alloca =
                build_wrappers::w_alloca(&self.ir.builder, result_ty.into(), "rf_err_slot");
            let tag_ptr_err = build_wrappers::w_struct_gep(
                &self.ir.builder,
                result_ty.into(),
                err_alloca,
            ...
            let payload_err = build_wrappers::w_struct_gep(
                &self.ir.builder,
                result_ty.into(),
                err_alloca,
                1,
            ...
            let str_err_ptr = build_wrappers::w_pointer_cast(
                &self.ir.builder,
                payload_err,
                str_ty.ptr_type(inkwell::AddressSpace::default()),
                "rf_str_err_ptr",
            ...
            let str_err_slot =
                build_wrappers::w_alloca(&self.ir.builder, str_ty.into(), "rf_str_err");
            build_wrappers::w_store(
                &self.ir.builder,
                build_wrappers::w_struct_gep(&self.ir.builder, str_ty.into(), str_err_slot, 0, ""),
                actual_len.into(),
            );
            build_wrappers::w_store(
                &self.ir.builder,
                build_wrappers::w_struct_gep(&self.ir.builder, str_ty.into(), str_err_slot, 1, ""),
                out_ptr.into(),
            );
            let str_err_val = build_wrappers::w_load(
                &self.ir.builder,
                str_ty.into(),
                str_err_slot,
                "rf_str_err_val",
        ...
        // (an i8* dict handle instead of a str path).
        {
            ...
            let rt_fn = self
                .ir
                .module
                .add_function("__axon_dict_to_str", rt_ty, None);
            ...
            let out_len_slot =
                build_wrappers::w_alloca(&self.ir.builder, i64_ty.into(), "dts_out_len");
            let out_ptr_slot =
                build_wrappers::w_alloca(&self.ir.builder, i8_ptr.into(), "dts_out_ptr");
            let out_ptr_cast = build_wrappers::w_pointer_cast(
                &self.ir.builder,
                out_ptr_slot,
                i8_ptr_ptr,
            ...
            build_wrappers::w_call(
                &self.ir.builder,
                rt_fn,
                &[d_arg.into(), out_len_slot.into(), out_ptr_cast.into()],
                "",
            );

            let out_len =
                build_wrappers::w_load(&self.ir.builder, i64_ty.into(), out_len_slot, "dts_len")
                    .into_int_value();
            let out_ptr =
                build_wrappers::w_load(&self.ir.builder, i8_ptr.into(), out_ptr_slot, "dts_ptr")
                    .into_pointer_value();
            let zero_i64 = i64_ty.const_int(0, false);
            let is_ok = build_wrappers::w_int_compare(
                &self.ir.builder,
            ...
            self.ir.builder.position_at_end(ok_bb);
            let ok_alloca =
                build_wrappers::w_alloca(&self.ir.builder, result_ty.into(), "dts_ok_slot");
            build_wrappers::w_store(
                &self.ir.builder,
                build_wrappers::w_struct_gep(
                    &self.ir.builder,
                    result_ty.into(),
                    ok_alloca,
                    0,
            ...
            let str_ok_slot =
                build_wrappers::w_alloca(&self.ir.builder, str_ty.into(), "dts_str_ok");
            build_wrappers::w_store(
                &self.ir.builder,
                build_wrappers::w_struct_gep(&self.ir.builder, str_ty.into(), str_ok_slot, 0, ""),
                out_len.into(),
            ...
            build_wrappers::w_store(&self.ir.builder, str_ok_ptr, str_ok_val);
            let ok_val =
                build_wrappers::w_load(&self.ir.builder, result_ty.into(), ok_alloca, "dts_ok_val");
            build_wrappers::w_ret(&self.ir.builder, ok_val);
            ...
            let err_alloca =
                build_wrappers::w_alloca(&self.ir.builder, result_ty.into(), "dts_err_slot");
            build_wrappers::w_store(
                &self.ir.builder,
                build_wrappers::w_struct_gep(
                    &self.ir.builder,
            ...
            let str_err_slot =
                build_wrappers::w_alloca(&self.ir.builder, str_ty.into(), "dts_str_err");
            build_wrappers::w_store(
                &self.ir.builder,
                build_wrappers::w_struct_gep(&self.ir.builder, str_ty.into(), str_err_slot, 0, ""),
                actual_len.into(),
        ...
        // {i64 len, ptr→[str struct]}; we pass its data ptr + len through.
        {
            ...
            let args_len =
                build_wrappers::w_extract_value(&self.ir.builder, args_arr, 0, "ex_alen")
                    .into_int_value();
            let args_data =
                build_wrappers::w_extract_value(&self.ir.builder, args_arr, 1, "ex_adata")
                    .into_pointer_value();

            let out_len_slot =
                build_wrappers::w_alloca(&self.ir.builder, i64_ty.into(), "ex_out_len");
            let out_ptr_slot =
                build_wrappers::w_alloca(&self.ir.builder, i8_ptr.into(), "ex_out_ptr");
            let out_ptr_cast = build_wrappers::w_pointer_cast(
                &self.ir.builder,
                out_ptr_slot,
                i8_ptr_ptr,
            ...
            let out_len =
                build_wrappers::w_load(&self.ir.builder, i64_ty.into(), out_len_slot, "ex_len")
                    .into_int_value();
            let out_ptr =
                build_wrappers::w_load(&self.ir.builder, i8_ptr.into(), out_ptr_slot, "ex_ptr")
                    .into_pointer_value();
            let is_ok = build_wrappers::w_int_compare(
                &self.ir.builder,
                inkwell::IntPredicate::SGE,
            ...
            self.ir.builder.position_at_end(ok_bb);
            let ok_alloca =
                build_wrappers::w_alloca(&self.ir.builder, result_ty.into(), "ex_ok_slot");
            build_wrappers::w_store(
                &self.ir.builder,
                build_wrappers::w_struct_gep(
                    &self.ir.builder,
            ...
            let str_ok_slot =
                build_wrappers::w_alloca(&self.ir.builder, str_ty.into(), "ex_str_ok");
            build_wrappers::w_store(
                &self.ir.builder,
                build_wrappers::w_struct_gep(&self.ir.builder, str_ty.into(), str_ok_slot, 0, ""),
                out_len.into(),
            ...
            build_wrappers::w_store(&self.ir.builder, str_ok_ptr, str_ok_val);
            let ok_val =
                build_wrappers::w_load(&self.ir.builder, result_ty.into(), ok_alloca, "ex_ok_val");
            build_wrappers::w_ret(&self.ir.builder, ok_val);
            ...
            let err_alloca =
                build_wrappers::w_alloca(&self.ir.builder, result_ty.into(), "ex_err_slot");
            build_wrappers::w_store(
                &self.ir.builder,
                build_wrappers::w_struct_gep(
                    &self.ir.builder,
            ...
            let str_err_slot =
                build_wrappers::w_alloca(&self.ir.builder, str_ty.into(), "ex_str_err");
            build_wrappers::w_store(
                &self.ir.builder,
                build_wrappers::w_struct_gep(&self.ir.builder, str_ty.into(), str_err_slot, 0, ""),
                actual_len.into(),
        ...
        // err_len==0 → Ok(()); err_len>0 → Err(str{err_len, err_ptr})
        {
            ...
            let rt_fn = self
                .ir
                .module
                .add_function("__axon_write_file", rt_ty, None);
            ...
            let path_len =
                build_wrappers::w_extract_value(&self.ir.builder, path_str, 0, "wf_plen")
                    .into_int_value();
            let path_ptr_v =
                build_wrappers::w_extract_value(&self.ir.builder, path_str, 1, "wf_pptr")
                    .into_pointer_value();
            let cont_len =
                build_wrappers::w_extract_value(&self.ir.builder, content_str, 0, "wf_clen")
                    .into_int_value();
            let cont_ptr =
                build_wrappers::w_extract_value(&self.ir.builder, content_str, 1, "wf_cptr")
                    .into_pointer_value();

            let err_len_slot =
                build_wrappers::w_alloca(&self.ir.builder, i64_ty.into(), "wf_err_len");
            let err_ptr_slot =
                build_wrappers::w_alloca(&self.ir.builder, i8_ptr.into(), "wf_err_ptr");
            let err_ptr_cast = build_wrappers::w_pointer_cast(
                &self.ir.builder,
                err_ptr_slot,
                i8_ptr_ptr,
                "wf_ptrptr",
            ...
            let err_len = build_wrappers::w_load(
                &self.ir.builder,
                i64_ty.into(),
                err_len_slot,
                "wf_err_len_val",
            ...
            self.ir.builder.position_at_end(ok_bb);
            let ok_alloca =
                build_wrappers::w_alloca(&self.ir.builder, result_ty.into(), "wf_ok_slot");
            let tag_ptr_ok = build_wrappers::w_struct_gep(
                &self.ir.builder,
                result_ty.into(),
                ok_alloca,
            ...
            let ok_val =
                build_wrappers::w_load(&self.ir.builder, result_ty.into(), ok_alloca, "wf_ok_val");
            build_wrappers::w_ret(&self.ir.builder, ok_val);

            // err_bb: { tag=0, payload=str{err_len, err_ptr} }
            self.ir.builder.position_at_end(err_bb);
            let err_ptr = build_wrappers::w_load(
                &self.ir.builder,
                i8_ptr.into(),
                err_ptr_slot,
                "wf_err_ptr_val",
            ...
            .into_pointer_value();
            let err_alloca =
                build_wrappers::w_alloca(&self.ir.builder, result_ty.into(), "wf_err_slot");
            let tag_ptr_err = build_wrappers::w_struct_gep(
                &self.ir.builder,
                result_ty.into(),
                err_alloca,
            ...
            let str_err_slot =
                build_wrappers::w_alloca(&self.ir.builder, str_ty.into(), "wf_str_err");
            build_wrappers::w_store(
                &self.ir.builder,
                build_wrappers::w_struct_gep(&self.ir.builder, str_ty.into(), str_err_slot, 0, ""),
                err_len.into(),
        ...
        for (fname, is_min) in &[("min_f64", true), ("max_f64", false)] {
            ...
            let result =
                build_wrappers::w_select(&self.ir.builder, cmp, a.into(), b.into(), "mf_result");
        ...
        // (bool=1 byte, str=16 bytes → max=16; same layout as Result<f64,str>)
        {
            ...
            let result_ty = self
                .ir
                .context
                .struct_type(&[i1_ty.into(), i8_arr16_ty.into()], false);
            ...
            let rt_fn = self
                .ir
                .module
                .get_function("__axon_parse_bool")
                .unwrap_or_else(|| {
                    self.ir
                        .module
                        .add_function("__axon_parse_bool", rt_ty, None)
                });
            let out_ok_slot =
                build_wrappers::w_alloca(&self.ir.builder, i64_ty.into(), "pb_ok_slot");
            let out_val_slot =
                build_wrappers::w_alloca(&self.ir.builder, i64_ty.into(), "pb_val_slot");
            let out_len_slot =
                build_wrappers::w_alloca(&self.ir.builder, i64_ty.into(), "pb_len_slot");
            let out_ptr_slot =
                build_wrappers::w_alloca(&self.ir.builder, i8_ptr.into(), "pb_ptr_slot");
            let out_ptr_slot_cast = build_wrappers::w_pointer_cast(
                &self.ir.builder,
                out_ptr_slot,
                i8_ptr_ptr,
            ...
            let ok_flag =
                build_wrappers::w_load(&self.ir.builder, i64_ty.into(), out_ok_slot, "pb_okflag")
                    .into_int_value();
            let zero = i64_ty.const_int(0, false);
            let is_ok = build_wrappers::w_int_compare(
                &self.ir.builder,
            ...
            self.ir.builder.position_at_end(ok_bb);
            let val_i64 =
                build_wrappers::w_load(&self.ir.builder, i64_ty.into(), out_val_slot, "pb_valint")
                    .into_int_value();
            let val_i1 = build_wrappers::w_int_compare(
                &self.ir.builder,
                inkwell::IntPredicate::NE,
                val_i64,
                zero,
            ...
            let ok_alloca =
                build_wrappers::w_alloca(&self.ir.builder, result_ty.into(), "pb_ok_alloca");
            build_wrappers::w_store(
                &self.ir.builder,
                build_wrappers::w_struct_gep(
                    &self.ir.builder,
            ...
            let bool_ptr = build_wrappers::w_pointer_cast(
                &self.ir.builder,
                payload_ok,
                i1_ty.ptr_type(inkwell::AddressSpace::default()),
                "pb_ot_bptr",
            );
            build_wrappers::w_store(&self.ir.builder, bool_ptr, val_i1.into());
            let ok_val =
                build_wrappers::w_load(&self.ir.builder, result_ty.into(), ok_alloca, "pb_ot_val");
            build_wrappers::w_ret(&self.ir.builder, ok_val);

            // err_bb: { tag=0, payload = str{out_len, out_ptr} }
            self.ir.builder.position_at_end(err_bb);
            let msg_len =
                build_wrappers::w_load(&self.ir.builder, i64_ty.into(), out_len_slot, "pb_emlen")
                    .into_int_value();
            let msg_ptr =
                build_wrappers::w_load(&self.ir.builder, i8_ptr.into(), out_ptr_slot, "pb_emptr")
                    .into_pointer_value();
            let err_alloca =
                build_wrappers::w_alloca(&self.ir.builder, result_ty.into(), "pb_err_alloca");
            build_wrappers::w_store(
                &self.ir.builder,
                build_wrappers::w_struct_gep(
                    &self.ir.builder,
            ...
            let str_ptr = build_wrappers::w_pointer_cast(
                &self.ir.builder,
                payload_err,
                str_ty.ptr_type(inkwell::AddressSpace::default()),
                "pb_err_sptr",
            ...
            let err_str_alloca =
                build_wrappers::w_alloca(&self.ir.builder, str_ty.into(), "pb_err_s");
            build_wrappers::w_store(
                &self.ir.builder,
                build_wrappers::w_struct_gep(
                    &self.ir.builder,
            ...
            let err_str_val =
                build_wrappers::w_load(&self.ir.builder, str_ty.into(), err_str_alloca, "pb_esv");
        ...
        //   • else      → lo + (rand() mod range), normalised non-negative.
        {
            ...
            let rip = self
                .ir
                .module
                .get_function("__axon_random_inverted_panic")
                .unwrap_or_else(|| {
                    self.ir
                        .module
                        .add_function("__axon_random_inverted_panic", rip_ty, None)
                });
            ...
            let is_empty =
                build_wrappers::w_int_compare(&self.ir.builder, IntPredicate::EQ, hi, lo, "ri_eq");
            ...
            let r_final =
                build_wrappers::w_int_signed_rem(&self.ir.builder, r_pos, range, "ri_final");
            ...
            self.fn_return_types
                .insert("random_i64".to_string(), Type::I64);
        }
        ...
        // Returns rand() / (RAND_MAX + 1.0) in [0.0, 1.0).
        {
            ...
            // Convert to f64
            let r_f =
                build_wrappers::w_signed_int_to_float(&self.ir.builder, r_i32, f64_ty, "rf_f");
            ...
            self.fn_return_types
                .insert("random_f64".to_string(), Type::F64);
        }
        ...
        // interpreter is the oracle (I-2); the runtime now matches it.
        {
            ...
            let rt_fn_ty = self
                ...
                .context
                .void_type()
                .fn_type(&[str_ty.into(), i64_ptr.into(), i8_ptr_ptr.into()], false);
            let rt_fn = self
                .ir
                .module
                .get_function("__axon_str_reverse")
                .unwrap_or_else(|| {
                    self.ir
                        .module
                        .add_function("__axon_str_reverse", rt_fn_ty, None)
                });
            ...
            let out_len_slot =
                build_wrappers::w_alloca(&self.ir.builder, i64_ty.into(), "srev_olen");
            let out_ptr_slot =
                build_wrappers::w_alloca(&self.ir.builder, i8_ptr.into(), "srev_optr");
            let out_ptr_slot_cast = build_wrappers::w_pointer_cast(
                &self.ir.builder,
                out_ptr_slot,
                i8_ptr_ptr,
            ...
            let out_len =
                build_wrappers::w_load(&self.ir.builder, i64_ty.into(), out_len_slot, "srev_len")
                    .into_int_value();
            let out_ptr =
                build_wrappers::w_load(&self.ir.builder, i8_ptr.into(), out_ptr_slot, "srev_ptr")
                    .into_pointer_value();
            let mut result = str_ty.const_zero();
            result = build_wrappers::w_insert_value(
                &self.ir.builder,
                result,
                out_len.into(),
                0,
            ...
            self.fn_return_types
                .insert("str_reverse".to_string(), Type::Str);
        }
        ...
        // __axon_str_digits_only (matches the interpreter's char filter).
        {
            ...
            let rt_fn = self
                .ir
                .module
                .get_function("__axon_str_digits_only")
                .unwrap_or_else(|| {
                    self.ir
                        .module
                        .add_function("__axon_str_digits_only", rt_fn_ty, None)
                });
            ...
            let s_arg = fn_val.get_nth_param(0).unwrap();
            let out_len_slot =
                build_wrappers::w_alloca(&self.ir.builder, i64_ty.into(), "sdo_olen");
            let out_ptr_slot =
                build_wrappers::w_alloca(&self.ir.builder, i8_ptr.into(), "sdo_optr");
            let out_ptr_slot_cast = build_wrappers::w_pointer_cast(
                &self.ir.builder,
                out_ptr_slot,
                i8_ptr_ptr,
            ...
            let out_len =
                build_wrappers::w_load(&self.ir.builder, i64_ty.into(), out_len_slot, "sdo_len")
                    .into_int_value();
            let out_ptr =
                build_wrappers::w_load(&self.ir.builder, i8_ptr.into(), out_ptr_slot, "sdo_ptr")
                    .into_pointer_value();
            let mut result = str_ty.const_zero();
            result = build_wrappers::w_insert_value(
                &self.ir.builder,
            ...
            self.fn_return_types
                .insert("str_digits_only".to_string(), Type::Str);
        }
        ...
        // (i64 len, AxonStr* data) + the sep str struct + str out-params.
        {
            ...
            self.fn_return_types
                .insert("str_join".to_string(), Type::Str);
        }
        ...
        // Delegates to __axon_i64_to_str_radix in the runtime via out-params.
        {
            ...
            let rt_fn = self
                ...
                .unwrap_or_else(|| {
                    self.ir
                        .module
                        .add_function("__axon_i64_to_str_radix", rt_fn_ty, None)
                });
            ...
            let out_len_slot =
                build_wrappers::w_alloca(&self.ir.builder, i64_ty.into(), "radix_olen");
            let out_ptr_slot =
                build_wrappers::w_alloca(&self.ir.builder, i8_ptr.into(), "radix_optr");
            ...
            let out_len =
                build_wrappers::w_load(&self.ir.builder, i64_ty.into(), out_len_slot, "radix_len")
                    .into_int_value();
            let out_ptr =
                build_wrappers::w_load(&self.ir.builder, i8_ptr.into(), out_ptr_slot, "radix_ptr")
                    .into_pointer_value();
            ...
            self.functions
                .insert("i64_to_str_radix".to_string(), fn_val);
            self.fn_return_types
                .insert("i64_to_str_radix".to_string(), Type::Str);
        }
    ...
    pub(super) fn declare_asi_runtime_builtins(&mut self) {
        ...
        // visible builtin — only the codegen invokes it.
        {
            ...
            self.ir
                .module
                .add_function("__axon_provenance_log", prov_ty, None);
        }
        ...
        // `__axon_goal_run` can compute a best-observed score.
        {
            let f64_ty = self.ir.context.f64_type();
            let prov_i64_ty =
                void_ty.fn_type(&[i8_ptr.into(), i64_ty.into(), i64_ty.into()], false);
            self.ir
                .module
                .add_function("__axon_provenance_log_ret_i64", prov_i64_ty, None);
            ...
            self.ir
                .module
                .add_function("__axon_provenance_log_ret_i64_in", prov_i64_in_ty, None);

            let prov_f64_ty =
                void_ty.fn_type(&[i8_ptr.into(), i64_ty.into(), f64_ty.into()], false);
            self.ir
                .module
                .add_function("__axon_provenance_log_ret_f64", prov_f64_ty, None);
        }
        ...
        // lattice (e.g. confidence flowing in from `ai_extract_uncertain_*`).
        {
            ...
            let vp_fn = self
                .ir
                .module
                .add_function("__axon_verify_panic", vp_ty, None);
        ...
        // diverging from the interpreter's text; this prints the same line.
        {
            ...
            let _ = self
                .ir
                .module
                .add_function("__axon_msg_panic_i64", mpi_ty, None);
        ...
        {
            let reg_ty = void_ty.fn_type(&[i8_ptr.into(), i64_ty.into(), i8_ptr.into()], false);
            self.ir
                .module
                .add_function("__axon_register_adaptive", reg_ty, None);
            ...
            self.ir
                .module
                .add_function("__axon_register_goal_name", reg_goal_ty, None);
            ...
            self.ir
                .module
                .add_function("__axon_set_provenance_source", set_src_ty, None);
            ...
            self.ir
                .module
                .add_function("__axon_log_agent_action", log_aa_ty, None);
        }
        ...
        // (preserves Layer-1 stub behaviour for adaptive_basic.ax and similar).
        {
            ...
            let fn_ty = f64_ty.fn_type(&[str_ty.into(), f64_ty.into(), i64_ty.into()], false);
            ...
            let name_len =
                build_wrappers::w_extract_value(&self.ir.builder, name_str, 0, "gr_nlen")
                    .into_int_value();
            let name_ptr =
                build_wrappers::w_extract_value(&self.ir.builder, name_str, 1, "gr_nptr")
                    .into_pointer_value();
            ...
            let out_slot =
                build_wrappers::w_alloca(&self.ir.builder, f64_ty.into(), "gr_out_score");
            build_wrappers::w_call(
                &self.ir.builder,
                rt_fn,
                &[
            ...
            let score =
                build_wrappers::w_load(&self.ir.builder, f64_ty.into(), out_slot, "gr_score");
            ...
            self.fn_return_types
                .insert("goal_run".to_string(), Type::F64);
        }
    }
    ...
    pub(super) fn declare_ai_builtins(&mut self) {
        ...
        // Same out-param ABI as __axon_read_file: out_len<0 on error.
        {
            ...
            let rt_fn = self
                .ir
                .module
                .add_function("__axon_ai_complete", rt_ty, None);
            ...
            let prompt_len =
                build_wrappers::w_extract_value(&self.ir.builder, prompt_str, 0, "aic_plen")
                    .into_int_value();
            let prompt_ptr_v =
                build_wrappers::w_extract_value(&self.ir.builder, prompt_str, 1, "aic_pptr")
                    .into_pointer_value();

            let out_len_slot =
                build_wrappers::w_alloca(&self.ir.builder, i64_ty.into(), "aic_out_len");
            let out_ptr_slot =
                build_wrappers::w_alloca(&self.ir.builder, i8_ptr.into(), "aic_out_ptr");
            let out_ptr_cast = build_wrappers::w_pointer_cast(
                &self.ir.builder,
                out_ptr_slot,
                i8_ptr_ptr,
            ...
            let out_len =
                build_wrappers::w_load(&self.ir.builder, i64_ty.into(), out_len_slot, "aic_len")
                    .into_int_value();
            let out_ptr =
                build_wrappers::w_load(&self.ir.builder, i8_ptr.into(), out_ptr_slot, "aic_ptr")
                    .into_pointer_value();
            let zero_i64 = i64_ty.const_int(0, false);
            let is_ok = build_wrappers::w_int_compare(
                &self.ir.builder,
            ...
            self.ir.builder.position_at_end(ok_bb);
            let ok_alloca =
                build_wrappers::w_alloca(&self.ir.builder, result_ty.into(), "aic_ok_slot");
            let tag_ptr_ok = build_wrappers::w_struct_gep(
                &self.ir.builder,
                result_ty.into(),
                ok_alloca,
            ...
            let str_ok_slot =
                build_wrappers::w_alloca(&self.ir.builder, str_ty.into(), "aic_str_ok");
            build_wrappers::w_store(
                &self.ir.builder,
                build_wrappers::w_struct_gep(&self.ir.builder, str_ty.into(), str_ok_slot, 0, ""),
                out_len.into(),
            ...
            build_wrappers::w_store(&self.ir.builder, str_ok_ptr, str_ok_val);
            let ok_val =
                build_wrappers::w_load(&self.ir.builder, result_ty.into(), ok_alloca, "aic_ok_val");
            build_wrappers::w_ret(&self.ir.builder, ok_val);
            ...
            let err_alloca =
                build_wrappers::w_alloca(&self.ir.builder, result_ty.into(), "aic_err_slot");
            let tag_ptr_err = build_wrappers::w_struct_gep(
                &self.ir.builder,
                result_ty.into(),
                err_alloca,
            ...
            let str_err_slot =
                build_wrappers::w_alloca(&self.ir.builder, str_ty.into(), "aic_str_err");
            build_wrappers::w_store(
                &self.ir.builder,
                build_wrappers::w_struct_gep(&self.ir.builder, str_ty.into(), str_err_slot, 0, ""),
                actual_len.into(),
        ...
        // source_tag = 1 (`from AI`); 0 is reserved for user-constructed.
        {
            ...
            let result_unc_i64_ty = self
                .ir
                .context
                .struct_type(&[bool_ty.into(), i8_arr24_ty.into()], false);
            ...
            // ── ai_extract_uncertain_i64 ──
            {
                ...
                let fn_val = self
                    .ir
                    .module
                    .add_function("ai_extract_uncertain_i64", fn_ty, None);
                ...
                let prompt_str = fn_val.get_nth_param(0).unwrap().into_struct_value();
                let prompt_len =
                    build_wrappers::w_extract_value(&self.ir.builder, prompt_str, 0, "aei_plen")
                        .into_int_value();
                let prompt_ptr_v =
                    build_wrappers::w_extract_value(&self.ir.builder, prompt_str, 1, "aei_pptr")
                        .into_pointer_value();

                let out_val_slot =
                    build_wrappers::w_alloca(&self.ir.builder, i64_ty.into(), "aei_out_val");
                let out_conf_slot =
                    build_wrappers::w_alloca(&self.ir.builder, f64_ty.into(), "aei_out_conf");
                let out_err_len_slot =
                    build_wrappers::w_alloca(&self.ir.builder, i64_ty.into(), "aei_out_err_len");
                let out_err_ptr_slot =
                    build_wrappers::w_alloca(&self.ir.builder, i8_ptr.into(), "aei_out_err_ptr");
                let out_err_ptr_cast = build_wrappers::w_pointer_cast(
                    &self.ir.builder,
                    out_err_ptr_slot,
                    i8_ptr_ptr,
                    "aei_eptrptr",
                ...
                let rc_call = build_wrappers::w_call(
                    &self.ir.builder,
                    rt_fn,
                    &[
                        prompt_ptr_v.into(),
                ...
                .into_int_value();
                let conf = build_wrappers::w_load(
                    &self.ir.builder,
                    f64_ty.into(),
                    out_conf_slot,
                    "aei_conf",
                ...
                unc_sv = build_wrappers::w_insert_value(
                    &self.ir.builder,
                    unc_sv,
                    val.into(),
                    0,
                ...
                .into_struct_value();
                let ok_alloca = build_wrappers::w_alloca(
                    &self.ir.builder,
                    result_unc_i64_ty.into(),
                    "aei_ok_slot",
                );
                let tag_ptr_ok = build_wrappers::w_struct_gep(
                    &self.ir.builder,
                ...
                let unc_payload_ptr = build_wrappers::w_pointer_cast(
                    &self.ir.builder,
                    payload_ok,
                    unc_i64_ty.ptr_type(inkwell::AddressSpace::default()),
                    "aei_unc_pp",
                );
                ...
                .into_pointer_value();
                let err_alloca = build_wrappers::w_alloca(
                    &self.ir.builder,
                    result_unc_i64_ty.into(),
                    "aei_err_slot",
                );
                ...
                let str_err_slot =
                    build_wrappers::w_alloca(&self.ir.builder, str_ty.into(), "aei_str_err");
                build_wrappers::w_store(
                    &self.ir.builder,
                    build_wrappers::w_struct_gep(
                        &self.ir.builder,
                ...
                self.functions
                    .insert("ai_extract_uncertain_i64".to_string(), fn_val);
                self.fn_return_types.insert(
            ...
            // ── ai_extract_uncertain_f64 ──
            {
                ...
                let fn_val = self
                    .ir
                    .module
                    .add_function("ai_extract_uncertain_f64", fn_ty, None);
                ...
                let prompt_str = fn_val.get_nth_param(0).unwrap().into_struct_value();
                let prompt_len =
                    build_wrappers::w_extract_value(&self.ir.builder, prompt_str, 0, "aef_plen")
                        .into_int_value();
                let prompt_ptr_v =
                    build_wrappers::w_extract_value(&self.ir.builder, prompt_str, 1, "aef_pptr")
                        .into_pointer_value();

                let out_val_slot =
                    build_wrappers::w_alloca(&self.ir.builder, f64_ty.into(), "aef_out_val");
                let out_conf_slot =
                    build_wrappers::w_alloca(&self.ir.builder, f64_ty.into(), "aef_out_conf");
                let out_err_len_slot =
                    build_wrappers::w_alloca(&self.ir.builder, i64_ty.into(), "aef_out_err_len");
                let out_err_ptr_slot =
                    build_wrappers::w_alloca(&self.ir.builder, i8_ptr.into(), "aef_out_err_ptr");
                let out_err_ptr_cast = build_wrappers::w_pointer_cast(
                    &self.ir.builder,
                    out_err_ptr_slot,
                    i8_ptr_ptr,
                ...
                let unc_payload_ptr = build_wrappers::w_pointer_cast(
                    &self.ir.builder,
                    payload_ok,
                    unc_f64_ty.ptr_type(inkwell::AddressSpace::default()),
                    "aef_unc_pp",
                );
                build_wrappers::w_store(&self.ir.builder, unc_payload_ptr, unc_sv.into());
                let ok_val = build_wrappers::w_load(
                ...
                let str_err_slot =
                    build_wrappers::w_alloca(&self.ir.builder, str_ty.into(), "aef_str_err");
                build_wrappers::w_store(
                    &self.ir.builder,
                    build_wrappers::w_struct_gep(
                        &self.ir.builder,
                ...
                self.functions
                    .insert("ai_extract_uncertain_f64".to_string(), fn_val);
                self.fn_return_types.insert(
        ...
        // is 16).  source_tag is *not* present here because T is a flat scalar.
        {
            ...
            let mut emit_flat_helper = |val_kind: FlatVal,
                                        helper_name: &str,
                                        rt_name: &str,
                                        axon_t: Type| {
                let (val_llvm_ty, val_ptr_ty): (
                    BasicTypeEnum<'ctx>,
                ...
                let rt_fn = self
                    .ir
                    .module
                    .add_function(rt_name, make_rt_ty(val_ptr_ty), None);
                ...
                let prompt_str = fn_val.get_nth_param(0).unwrap().into_struct_value();
                let prompt_len =
                    build_wrappers::w_extract_value(&self.ir.builder, prompt_str, 0, "aex_plen")
                        .into_int_value();
                let prompt_ptr_v =
                    build_wrappers::w_extract_value(&self.ir.builder, prompt_str, 1, "aex_pptr")
                        .into_pointer_value();

                let out_val_slot =
                    build_wrappers::w_alloca(&self.ir.builder, val_llvm_ty, "aex_out_val");
                let out_err_len_slot =
                    build_wrappers::w_alloca(&self.ir.builder, i64_ty.into(), "aex_out_err_len");
                let out_err_ptr_slot =
                    build_wrappers::w_alloca(&self.ir.builder, i8_ptr.into(), "aex_out_err_ptr");
                let out_err_ptr_cast = build_wrappers::w_pointer_cast(
                    &self.ir.builder,
                    out_err_ptr_slot,
                    i8_ptr_ptr,
                ...
                let typed_payload_ptr = build_wrappers::w_pointer_cast(
                    &self.ir.builder,
                    payload_ok,
                    val_ptr_ty,
                    "aex_typed_pp",
                ...
                let val_loaded =
                    build_wrappers::w_load(&self.ir.builder, val_llvm_ty, out_val_slot, "aex_val");
                ...
                let str_err_slot =
                    build_wrappers::w_alloca(&self.ir.builder, str_ty.into(), "aex_str_err");
                build_wrappers::w_store(
                    &self.ir.builder,
                    build_wrappers::w_struct_gep(
                        &self.ir.builder,
            ...
            emit_flat_helper(
                FlatVal::I64,
                "ai_extract_i64",
                "__axon_ai_extract_i64",
                Type::I64,
            );
            emit_flat_helper(
                FlatVal::F64,
                "ai_extract_f64",
                "__axon_ai_extract_f64",
                Type::F64,
            );
            emit_flat_helper(
                FlatVal::Bool,
                "ai_extract_bool",
                "__axon_ai_extract_bool",
                Type::Bool,
            );
        }
    }
    ...
    pub(super) fn declare_phase9_math_builtins(&mut self) {
        ...
        // ── Phase 9: i64_to_f64(n: i64) -> f64 ──────────────────────────────
        {
            ...
            self.fn_return_types
                .insert("i64_to_f64".to_string(), Type::F64);
        }
        ...
        // ── Phase 9: f64_to_i64(x: f64) -> i64 ──────────────────────────────
        {
            ...
            self.fn_return_types
                .insert("f64_to_i64".to_string(), Type::I64);
        }
        ...
        // Use LLVM intrinsics via C libm linkage.
        {
            ...
            for (axon_name, c_name) in &[
                ...
                self.fn_return_types
                    .insert(axon_name.to_string(), Type::F64);
            }
        }
    }
    ...
    pub(super) fn declare_string_builtins(&mut self) {
        ...
        // Compare two strings for byte-equal content. Uses memcmp after length check.
        {
            ...
            let a_len =
                build_wrappers::w_extract_value(&self.ir.builder, a, 0, "se_alen").into_int_value();
            let b_len =
                build_wrappers::w_extract_value(&self.ir.builder, b, 0, "se_blen").into_int_value();
            let a_ptr = build_wrappers::w_extract_value(&self.ir.builder, a, 1, "se_aptr")
                .into_pointer_value();
            let b_ptr = build_wrappers::w_extract_value(&self.ir.builder, b, 1, "se_bptr")
                .into_pointer_value();
            ...
            let memcmp_fn = self.ir.module.get_function("memcmp").unwrap_or_else(|| {
                let memcmp_ty =
                    i32_ty.fn_type(&[i8_ptr.into(), i8_ptr.into(), i64_ty.into()], false);
            ...
            self.fn_return_types
                .insert("str_eq".to_string(), Type::Bool);
        }
        ...
        // Migrated to axon-rt (R1 Batch 2b): out-param ABI via __axon_str_slice.
        {
            ...
            let rt_fn = self
                .ir
                .module
                .get_function("__axon_str_slice")
                .unwrap_or_else(|| {
                    self.ir
                        .module
                        .add_function("__axon_str_slice", rt_fn_ty, None)
                });
            ...
            let out_len_slot =
                build_wrappers::w_alloca(&self.ir.builder, i64_ty.into(), "sslice_olen");
            let out_ptr_slot =
                build_wrappers::w_alloca(&self.ir.builder, i8_ptr.into(), "sslice_optr");
            let out_ptr_slot_cast = build_wrappers::w_pointer_cast(
                &self.ir.builder,
                out_ptr_slot,
                i8_ptr_ptr,
            ...
            let out_len =
                build_wrappers::w_load(&self.ir.builder, i64_ty.into(), out_len_slot, "sslice_len")
                    .into_int_value();
            let out_ptr =
                build_wrappers::w_load(&self.ir.builder, i8_ptr.into(), out_ptr_slot, "sslice_ptr")
                    .into_pointer_value();
            ...
            self.fn_return_types
                .insert("str_slice".to_string(), Type::Str);
        }
        ...
        // Returns str "true" or "false" (global string constants).
        {
            ...
            let is_true = build_wrappers::w_int_compare(
                &self.ir.builder,
                inkwell::IntPredicate::NE,
                b,
            ...
            let true_bytes: Vec<_> = b"true\0"
                .iter()
                .map(|&c| self.ir.context.i8_type().const_int(c as u64, false))
                .collect();
            let false_bytes: Vec<_> = b"false\0"
                .iter()
                .map(|&c| self.ir.context.i8_type().const_int(c as u64, false))
                .collect();
            let true_g = self.ir.module.add_global(
                self.ir.context.i8_type().array_type(5),
                None,
                "tsb_true_str",
            );
            true_g.set_initializer(&self.ir.context.i8_type().const_array(&true_bytes));
            true_g.set_constant(true);
            let false_g = self.ir.module.add_global(
                self.ir.context.i8_type().array_type(6),
                None,
                "tsb_false_str",
            );
            ...
            self.ir.builder.position_at_end(true_bb);
            let true_ptr = build_wrappers::w_pointer_cast(
                &self.ir.builder,
            ...
            true_str = build_wrappers::w_insert_value(
                &self.ir.builder,
                true_str,
            ...
            self.ir.builder.position_at_end(false_bb);
            let false_ptr = build_wrappers::w_pointer_cast(
                &self.ir.builder,
            ...
            false_str = build_wrappers::w_insert_value(
                &self.ir.builder,
                false_str,
                i64_ty.const_int(5, false).into(),
                0,
                "tsb_f0",
            )
            .into_struct_value();
            ...
            self.fn_return_types
                .insert("to_str_bool".to_string(), Type::Str);
        }
        ...
        // Result<f64, str> = { i1, [16 x i8] } (f64=8 bytes, str=16 bytes → max=16)
        {
            ...
            let rt_fn = self
                .ir
                .module
                .get_function("__axon_parse_float")
                .unwrap_or_else(|| {
                    self.ir
                        .module
                        .add_function("__axon_parse_float", rt_ty, None)
                });
            let out_ok_slot =
                build_wrappers::w_alloca(&self.ir.builder, i64_ty.into(), "pf_ok_slot");
            let out_val_slot =
                build_wrappers::w_alloca(&self.ir.builder, f64_ty.into(), "pf_val_slot");
            let out_len_slot =
                build_wrappers::w_alloca(&self.ir.builder, i64_ty.into(), "pf_len_slot");
            let out_ptr_slot =
                build_wrappers::w_alloca(&self.ir.builder, i8_ptr.into(), "pf_ptr_slot");
            let out_ptr_slot_cast = build_wrappers::w_pointer_cast(
                &self.ir.builder,
                out_ptr_slot,
                i8_ptr_ptr,
            ...
            let ok_flag =
                build_wrappers::w_load(&self.ir.builder, i64_ty.into(), out_ok_slot, "pf_okflag")
                    .into_int_value();
            let zero = i64_ty.const_int(0, false);
            let is_ok = build_wrappers::w_int_compare(
                &self.ir.builder,
            ...
            self.ir.builder.position_at_end(ok_bb);
            let parsed_f64 =
                build_wrappers::w_load(&self.ir.builder, f64_ty.into(), out_val_slot, "pf_parsed")
                    .into_float_value();
            let ok_alloca =
                build_wrappers::w_alloca(&self.ir.builder, result_ty.into(), "pf_ok_alloca");
            let tag_ptr_ok = build_wrappers::w_struct_gep(
                &self.ir.builder,
                result_ty.into(),
                ok_alloca,
            ...
            let f64_ptr = build_wrappers::w_pointer_cast(
                &self.ir.builder,
                payload_ok,
                f64_ptr_ty,
                "pf_f64_ptr",
            ...
            let ok_val =
                build_wrappers::w_load(&self.ir.builder, result_ty.into(), ok_alloca, "pf_ok_val");
            build_wrappers::w_ret(&self.ir.builder, ok_val);

            // err_bb: { tag=0, payload=str{out_len, out_ptr} }
            self.ir.builder.position_at_end(err_bb);
            let msg_len =
                build_wrappers::w_load(&self.ir.builder, i64_ty.into(), out_len_slot, "pf_emlen")
                    .into_int_value();
            let msg_ptr =
                build_wrappers::w_load(&self.ir.builder, i8_ptr.into(), out_ptr_slot, "pf_emptr")
                    .into_pointer_value();
            let err_alloca =
                build_wrappers::w_alloca(&self.ir.builder, result_ty.into(), "pf_err_alloca");
            let tag_ptr_err = build_wrappers::w_struct_gep(
                &self.ir.builder,
                result_ty.into(),
                err_alloca,
            ...
            let err_str_ptr = build_wrappers::w_pointer_cast(
                &self.ir.builder,
                payload_err,
                str_ty.ptr_type(inkwell::AddressSpace::default()),
                "pf_str_err_ptr",
            ...
            let err_str_slot =
                build_wrappers::w_alloca(&self.ir.builder, str_ty.into(), "pf_str_err");
            build_wrappers::w_store(
                &self.ir.builder,
                build_wrappers::w_struct_gep(&self.ir.builder, str_ty.into(), err_str_slot, 0, ""),
                msg_len.into(),
            );
            build_wrappers::w_store(
                &self.ir.builder,
                build_wrappers::w_struct_gep(&self.ir.builder, str_ty.into(), err_str_slot, 1, ""),
                msg_ptr.into(),
            );
            let err_str_val = build_wrappers::w_load(
                &self.ir.builder,
        ...
        // i8*} AxonStr used as the first param of each parser.
        {
            ...
            let build_or = |slf: &mut Self,
                ...
                let tag_ptr =
                    build_wrappers::w_struct_gep(&slf.ir.builder, result_ty, r_slot, 0, "po_tagp");
                let tag = build_wrappers::w_load(
                    &slf.ir.builder,
                    slf.ir.context.bool_type().into(),
                    tag_ptr,
                    "po_tag",
                )
                .into_int_value();
                ...
                let pay_ptr =
                    build_wrappers::w_struct_gep(&slf.ir.builder, result_ty, r_slot, 1, "po_payp");
                let pay_val_ptr = build_wrappers::w_pointer_cast(
                    &slf.ir.builder,
                    pay_ptr,
                    val_ty.ptr_type(inkwell::AddressSpace::default()),
                    "po_payvp",
                );
                ...
                if let Some(b) = saved {
                    slf.ir.builder.position_at_end(b);
                }
        ...
        for (fname, rt_name) in &[
            ("str_to_upper", "__axon_str_to_upper"),
            ("str_to_lower", "__axon_str_to_lower"),
        ] {
            let str_ty = self
                .ir
                .context
            ...
            let s_arg = fn_val.get_nth_param(0).unwrap();
            let out_len_slot =
                build_wrappers::w_alloca(&self.ir.builder, i64_ty.into(), "stl_olen");
            let out_ptr_slot =
                build_wrappers::w_alloca(&self.ir.builder, i8_ptr.into(), "stl_optr");
            let out_ptr_slot_cast = build_wrappers::w_pointer_cast(
                &self.ir.builder,
                out_ptr_slot,
                i8_ptr_ptr,
            ...
            let out_len =
                build_wrappers::w_load(&self.ir.builder, i64_ty.into(), out_len_slot, "stl_len")
                    .into_int_value();
            let out_ptr =
                build_wrappers::w_load(&self.ir.builder, i8_ptr.into(), out_ptr_slot, "stl_ptr")
                    .into_pointer_value();
            let mut result = str_ty.const_zero();
            result = build_wrappers::w_insert_value(
                &self.ir.builder,
        ...
        for (fname, rt_name) in &[
            ...
            let s_arg = fn_val.get_nth_param(0).unwrap();
            let out_len_slot =
                build_wrappers::w_alloca(&self.ir.builder, i64_ty.into(), "stt_olen");
            let out_ptr_slot =
                build_wrappers::w_alloca(&self.ir.builder, i8_ptr.into(), "stt_optr");
            let out_ptr_slot_cast = build_wrappers::w_pointer_cast(
                &self.ir.builder,
                out_ptr_slot,
                i8_ptr_ptr,
            ...
            let out_len =
                build_wrappers::w_load(&self.ir.builder, i64_ty.into(), out_len_slot, "stt_len")
                    .into_int_value();
            let out_ptr =
                build_wrappers::w_load(&self.ir.builder, i8_ptr.into(), out_ptr_slot, "stt_ptr")
                    .into_pointer_value();
            let mut result = str_ty.const_zero();
            result = build_wrappers::w_insert_value(
                &self.ir.builder,
        ...
        // Migrated to axon-rt (R1 Batch 2b): out-param ABI via __axon_str_repeat.
        {
            ...
            let rt_fn = self
                .ir
                .module
                .get_function("__axon_str_repeat")
                .unwrap_or_else(|| {
                    self.ir
                        .module
                        .add_function("__axon_str_repeat", rt_fn_ty, None)
                });
            ...
            let out_len_slot =
                build_wrappers::w_alloca(&self.ir.builder, i64_ty.into(), "srep_olen");
            let out_ptr_slot =
                build_wrappers::w_alloca(&self.ir.builder, i8_ptr.into(), "srep_optr");
            let out_ptr_slot_cast = build_wrappers::w_pointer_cast(
                &self.ir.builder,
                out_ptr_slot,
                i8_ptr_ptr,
            ...
            let out_len =
                build_wrappers::w_load(&self.ir.builder, i64_ty.into(), out_len_slot, "srep_len")
                    .into_int_value();
            let out_ptr =
                build_wrappers::w_load(&self.ir.builder, i8_ptr.into(), out_ptr_slot, "srep_ptr")
                    .into_pointer_value();
        ...
        // "XaXbXcX". The runtime now matches the interpreter.
        {
            ...
            let rt_fn = self
                .ir
                .module
                .get_function("__axon_str_replace")
                .unwrap_or_else(|| {
                    self.ir
                        .module
                        .add_function("__axon_str_replace", rt_fn_ty, None)
                });
            ...
            let out_len_slot =
                build_wrappers::w_alloca(&self.ir.builder, i64_ty.into(), "srpl_olen");
            let out_ptr_slot =
                build_wrappers::w_alloca(&self.ir.builder, i8_ptr.into(), "srpl_optr");
            let out_ptr_slot_cast = build_wrappers::w_pointer_cast(
                &self.ir.builder,
                out_ptr_slot,
                i8_ptr_ptr,
            ...
            let out_len =
                build_wrappers::w_load(&self.ir.builder, i64_ty.into(), out_len_slot, "srpl_len")
                    .into_int_value();
            let out_ptr =
                build_wrappers::w_load(&self.ir.builder, i8_ptr.into(), out_ptr_slot, "srpl_ptr")
                    .into_pointer_value();
            let mut result = str_ty.const_zero();
            result = build_wrappers::w_insert_value(
                &self.ir.builder,
        ...
        // Calls C getenv(). Returns Ok(str) if set, Err("not set") otherwise.
        {
            ...
            let name_ptr = build_wrappers::w_extract_value(&self.ir.builder, name_s, 1, "ev_np")
                .into_pointer_value();

            let val_ptr =
                build_wrappers::w_call(&self.ir.builder, getenv_fn, &[name_ptr.into()], "ev_val")
                    .try_as_basic_value()
                    .left()
                    .unwrap()
                    .into_pointer_value();
            ...
            let null_ptr = i8_ptr.const_null();
            let is_null = build_wrappers::w_int_compare(
                &self.ir.builder,
                inkwell::IntPredicate::EQ,
            ...
            self.ir.builder.position_at_end(ok_bb);
            let val_len_raw =
                build_wrappers::w_call(&self.ir.builder, strlen_fn, &[val_ptr.into()], "ev_vlen")
                    .try_as_basic_value()
                    .left()
                    .unwrap()
                    .into_int_value();
            ...
            let ok_str_ptr =
                build_wrappers::w_alloca(&self.ir.builder, result_ty.into(), "ev_ok_r");
            let tag_gep = build_wrappers::w_struct_gep(
                &self.ir.builder,
                result_ty.into(),
                ok_str_ptr,
                0,
            ...
            let payload_gep = build_wrappers::w_struct_gep(
                &self.ir.builder,
                result_ty.into(),
                ok_str_ptr,
                1,
            ...
            let payload_as_str = build_wrappers::w_pointer_cast(
                &self.ir.builder,
                payload_gep,
                str_ty.ptr_type(inkwell::AddressSpace::default()),
                "ev_str_ptr",
            ...
            let ok_result =
                build_wrappers::w_load(&self.ir.builder, result_ty.into(), ok_str_ptr, "ev_ok_val");
            ...
            let err_global = self.ir.module.add_global(
                self.ir.context.i8_type().array_type(err_msg.len() as u32),
                None,
                "ev_err_str",
            );
            let err_bytes: Vec<_> = err_msg
                .bytes()
                .map(|c| self.ir.context.i8_type().const_int(c as u64, false))
                .collect();
            err_global.set_initializer(&self.ir.context.i8_type().const_array(&err_bytes));
            err_global.set_constant(true);
            let err_ptr = build_wrappers::w_pointer_cast(
                &self.ir.builder,
            ...
            let err_str_ptr =
                build_wrappers::w_alloca(&self.ir.builder, result_ty.into(), "ev_err_r");
            let tag_gep2 = build_wrappers::w_struct_gep(
                &self.ir.builder,
                result_ty.into(),
                err_str_ptr,
                0,
            ...
            let payload_gep2 = build_wrappers::w_struct_gep(
                &self.ir.builder,
                result_ty.into(),
                err_str_ptr,
                1,
            ...
            let payload_as_str2 = build_wrappers::w_pointer_cast(
                &self.ir.builder,
                payload_gep2,
                str_ty.ptr_type(inkwell::AddressSpace::default()),
                "ev_str_ptr2",
            ...
            let err_result = build_wrappers::w_load(
                &self.ir.builder,
                result_ty.into(),
                err_str_ptr,
                "ev_err_val",
            ...
            self.fn_return_types
                .insert("env_var".to_string(), result_type);
        }
        ...
        // ── Phase 6: exit(code: i64) -> () ───────────────────────────────────
        {
            let c_exit_fn = self.ir.module.get_function("exit").unwrap_or_else(|| {
                let ft = self
                    ...
                    .context
                    .void_type()
                    .fn_type(&[self.ir.context.i32_type().into()], false);
        ...
        for (fname, rt_name) in &[
            ...
            let out_len =
                build_wrappers::w_load(&self.ir.builder, i64_ty.into(), out_len_slot, "sp_len")
                    .into_int_value();
            let out_ptr =
                build_wrappers::w_load(&self.ir.builder, i8_ptr.into(), out_ptr_slot, "sp_ptr")
                    .into_pointer_value();
            let mut result = str_ty.const_zero();
            result = build_wrappers::w_insert_value(
                &self.ir.builder,
