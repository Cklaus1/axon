...
impl<'ctx> super::Codegen<'ctx> {
        ...
        match inner {
            Some(val) => {
                ...
                let opt_ty = self
                    .ir
                    .context
                    .struct_type(&[tag_ty.into(), llvm_inner], false);
                ...
                let val_ptr = build_wrappers::w_struct_gep(
                    &self.ir.builder,
                    opt_ty.into(),
                    alloca,
                    1,
            ...
            None => {
                ...
                if let Some(inner_llvm_ty) = llvm_inner {
                    let opt_ty = self
                        .ir
                        .context
                        .struct_type(&[tag_ty.into(), inner_llvm_ty], false);
    ...
    pub(super) fn emit_result(
        ...
        if let Some((ok_ty, err_ty)) = self.current_result_types.clone() {
            ...
            let result_ty = self
                .ir
                .context
                .struct_type(&[tag_ty.into(), payload_arr_ty.into()], false);
            ...
            let typed_pay_ptr =
                build_wrappers::w_pointer_cast(&self.ir.builder, pay_ptr, val_ptr_ty, "pay_typed");
            ...
            let result_ty = self
                .ir
                .context
                .struct_type(&[tag_ty.into(), payload_ty], false);
            let alloca = build_wrappers::w_alloca(&self.ir.builder, result_ty.into(), "result");
            let tag_ptr = build_wrappers::w_struct_gep(
                &self.ir.builder,
                result_ty.into(),
    ...
    pub(super) fn extract_result_payload(
        ...
        let typed_ptr =
            build_wrappers::w_pointer_cast(&self.ir.builder, arr_alloca, ptr_ty, "payloadptr");
    ...
    pub(super) fn emit_question(
        ...
        let tag = build_wrappers::w_extract_value(
            &self.ir.builder,
        ...
        let err_payload = build_wrappers::w_extract_value(
            &self.ir.builder,
            result_val.into_struct_value(),
            1,
            "qerr_payload",
        ...
        let err_return_val = if let Some((_, err_ty)) = self.current_result_types.clone() {
            ...
            let typed_err = self
                .extract_result_payload(err_payload, &err_ty)
        ...
        let raw_payload = build_wrappers::w_extract_value(
            &self.ir.builder,
            result_val.into_struct_value(),
            1,
            "qpayload",
