...
impl<'ctx> super::Codegen<'ctx> {
    ...
    /// linking. LLVM cross-emits the AArch64/x86_64 object even on Linux; the link
    /// into `.a`/`.xcframework` (iOS, macOS-only) or `.so` (Android, NDK) is the
    /// platform toolchain's job (spec §4 / §12 Q4).
