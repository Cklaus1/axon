...
enum Command {
    ...
    /// Emit the complete machine-readable reference for this build: every CLI
    /// verb, every builtin with its signature, every attribute.
    ///
    /// GENERATED FROM THE CODE, not from prose. The tables it reads —
    /// `BUILTINS`, `DEFERRED_ATTRS`, clap's own subcommand list — are the same
    /// ones the compiler uses, so the reference cannot describe a language this
    /// binary does not implement.
    ///
    /// It exists because the prose documentation could and did: the binary had
    /// 24 verbs while `CLAUDE.md` named 13, and `claims_gate.sh` checks only
    /// that a NAMED verb exists, never that an existing verb is named. A doc
    /// that can omit silently is one an agent reads as complete.
    Reference {
        /// Emit JSON (schema `axon-reference/1`) instead of Markdown.
...
fn dispatch(command: Command) {
    ...
    match command {
        ...
        Command::Reference { json } => cmd_reference(json),
        Command::Session {
            protocol,
            show_program,
            require_contained,
...
/// `axon reference` — the complete surface of THIS build, generated from the
/// tables the compiler itself uses.
///
/// Every section is derived, never transcribed. `CLAUDE.md` is a curated
/// selection and says so; this is the exhaustive counterpart, and the two answer
/// different questions. A reader who needs to know "does this build support X"
/// asks here.
fn cmd_reference(json: bool) {
    use clap::CommandFactory;
    let cmd = Cli::command();
    let mut verbs: Vec<(String, String)> = cmd
        .get_subcommands()
        .map(|c| {
            let about = c
                .get_about()
                .map(|a| a.to_string())
                .unwrap_or_default()
                // The first line only: a verb table wants one line per verb, and
                // the long help is available from `axon <verb> --help`.
                .lines()
                .next()
                .unwrap_or_default()
                .to_string();
        ...
        .collect();
    verbs.sort_by(|a, b| a.0.cmp(&b.0));

    let mut builtins: Vec<&axon_core::builtins::BuiltinFn> =
        axon_core::builtins::BUILTINS.iter().collect();
    ...
    if json {
        ...
        let vs: Vec<String> = verbs
            .iter()
            .map(|(n, a)| format!("{{\"name\":\"{}\",\"about\":\"{}\"}}", esc(n), esc(a)))
            .collect();
        let bs: Vec<String> = builtins
            .iter()
            .map(|b| {
                let ps: Vec<String> = b
                    .params
                    .iter()
                    .map(|(n, t)| format!("{{\"name\":\"{}\",\"type\":\"{}\"}}", esc(n), esc(t)))
                    .collect();
                format!(
                    "{{\"name\":\"{}\",\"params\":[{}],\"ret\":\"{}\",\"doc\":\"{}\"}}",
                    esc(b.name),
    ...
    println!("<!-- GENERATED by `axon reference`. Do not edit by hand.");
    println!("     Regenerate with: axon reference > AXON_REFERENCE.md");
    println!("     Gated by scripts/reference_gate.sh, which fails if this file");
    println!("     drifts from the binary in EITHER direction. -->");
    println!();
    ...
    println!();
    println!(
        "Generated from the compiler's own tables (`BUILTINS`, `DEFERRED_ATTRS`, the clap \
         subcommand list), so it cannot describe a language this binary does not implement. \
         `CLAUDE.md` is a curated selection and says so; this is the exhaustive counterpart."
    );
    println!();

    println!("## CLI verbs ({})", verbs.len());
    println!();
    println!("| Verb | Purpose |");
    println!("|---|---|");
    for (n, a) in &verbs {
        println!("| `axon {n}` | {a} |");
    }
    println!();
    println!("Run `axon <verb> --help` for flags and long-form help.");
    println!();
    ...
    println!("## Builtins ({})", builtins.len());
    println!();
    println!("| Signature | Purpose |");
    println!("|---|---|");
    for b in &builtins {
        let params: Vec<String> = b.params.iter().map(|(n, t)| format!("{n}: {t}")).collect();
        // Pipes would break the table; the full text is in `--json`.
        let doc = b.doc.replace('|', "\\|").replace('\n', " ");
        println!(
            "| `{}({}) -> {}` | {} |",
            b.name,
            params.join(", "),
