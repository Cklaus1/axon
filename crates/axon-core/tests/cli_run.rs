...
fn build_aborts_on_codegen_unsupported_builtin_e0910() {
    ...
    // arr_flatten is a known builtin that is NOT yet codegen-lowered (the tuple
    // ops enumerate/zip now are; flatten/chunk/group_by/partition are not).
    ...
    if codegen_present {
        assert!(
            msg.contains("E0910") && msg.contains("arr_flatten"),
