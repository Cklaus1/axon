...
fn build_aborts_on_codegen_unsupported_builtin_e0910() {
    ...
    // arr_chunk is a known builtin that is NOT yet codegen-lowered (enumerate/
    // zip/flatten now are; chunk/group_by/partition are not).
    std::fs::write(&f, "fn main() -> i64 { let a = [1, 2, 3, 4, 5]\n let b = arr_chunk(&a, 2)\n len(b) }\n").unwrap();
    ...
    if codegen_present {
        assert!(
            msg.contains("E0910") && msg.contains("arr_chunk"),
