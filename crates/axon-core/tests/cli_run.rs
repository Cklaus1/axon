...
fn build_aborts_on_codegen_unsupported_builtin_e0910() {
    ...
    // arr_sort_by is a known builtin that is NOT yet codegen-lowered (the
    // map/filter/fold/zip_with closure ops now are; sort/unique/group_by are not).
    std::fs::write(&f, "fn main() -> i64 { let a = [3, 1, 2]\n let b = arr_sort_by(&a, |x, y| x - y)\n b[0] }\n").unwrap();
    ...
    if codegen_present {
        assert!(
            msg.contains("E0910") && msg.contains("arr_sort_by"),
