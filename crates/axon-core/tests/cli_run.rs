...
#[test]
fn building_an_ai_program_without_asi_runtime_is_refused_not_silently_upgraded() {
    // O031. `axon-core`'s default features exclude `asi-runtime`, so the
    // interpreter refuses AI calls (E1300, exit 5). Native codegen linked
    // `axon-ai` unconditionally, so the SAME compiler produced an AOT binary
    // that dialled the real model — a capability escalation decided by which
    // execution path you took, not by any grant.
    //
    // The build must refuse, and must not leave a binary behind: a program that
    // would behave differently under `run` and under a native build should not
    // exist.
    let src = "fn main() -> i64 {\n    match ai_complete(\"hi\") {\n        \
               Ok(s) => { println(s) }\n        Err(e) => { println(e) }\n    }\n    0\n}\n";
    let f = tmp_ax("ai_no_runtime", src);
    let out_bin = std::env::temp_dir().join(format!("axon_ai_refuse_{}", std::process::id()));
    ...
    // This test is only meaningful in a build WITHOUT the feature; with it, the
    // capability is genuinely available to both paths and building is correct.
    if cfg!(feature = "asi-runtime") {
        return;
    }
    assert_ne!(out.status.code(), Some(0), "the build must not succeed: {combined}");
    assert!(combined.contains("E0910"), "must be the refusal class: {combined}");
    assert!(combined.contains("asi-runtime"), "must say how to fix it: {combined}");
    assert!(
        !out_bin.exists(),
        "a refused build must leave no binary at {}",
        out_bin.display()
    );
    let _ = std::fs::remove_file(&out_bin);
}
