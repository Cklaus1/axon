...
fn build_aborts_on_codegen_unsupported_builtin_e0910() {
    ...
    // arr_zip is a known builtin that is NOT yet codegen-lowered (enumerate now
    // is; zip/flatten/chunk/group_by/partition are not).
    std::fs::write(&f, "fn main() -> i64 { let a = [1, 2]\n let b = [3, 4]\n let c = arr_zip(a, b)\n len(c) }\n").unwrap();
    ...
    if codegen_present {
        assert!(
            msg.contains("E0910") && msg.contains("arr_zip"),
