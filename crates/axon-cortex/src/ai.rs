...
impl AiPatchGenerator {
    ...
    /// What the model is asked. Built from the body and the observed facts and
    ...
    fn prompt(&self, obs: &Observation, target: &SymbolRef, c: &PatchConstraints) -> String {
...
impl PatchGenerator for AiPatchGenerator {
    ...
    fn propose(
        ...
        let prompt = self.prompt(observation, target, constraints);
