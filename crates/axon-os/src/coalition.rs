...
impl std::fmt::Display for CoalitionBound {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "coalition bound exceeded on {}: rollup={} ceiling={} attempted={} members={}",
            self.axis, self.rollup, self.ceiling, self.attempted, self.member_count
        )
    }
}
...
impl Coalition {
    ...
    pub fn carve_for_member(&mut self, slot: usize, c: Carve) -> Result<(), CoalitionBound> {
        ...
        if r.total_compute.saturating_add(c.compute) > self.ceiling.total_compute {
            return Err(CoalitionBound {
                axis: "compute".into(),
                rollup: r.total_compute,
                ceiling: self.ceiling.total_compute,
                attempted: c.compute,
        ...
        if r.total_budget.saturating_add(c.budget) > self.ceiling.total_budget {
            return Err(CoalitionBound {
                axis: "budget".into(),
                rollup: r.total_budget,
                ceiling: self.ceiling.total_budget,
                attempted: c.budget,
                member_count: self.members.len(),
            });
        }
    ...
    pub fn propose_vote(
        ...
        if self.quorum_power_used.saturating_add(power) > self.ceiling.max_quorum_power {
            return Err(CoalitionBound {
                axis: "quorum_power".into(),
                rollup: self.quorum_power_used,
                ceiling: self.ceiling.max_quorum_power,
                attempted: power,
                member_count: self.members.len(),
            });
        }
    ...
    pub fn member_pid(&self, slot: usize) -> Option<&str> {
        self.members.get(slot).map(|s| s.as_str())
    }
    pub fn quorum_power_used(&self) -> u64 {
        self.quorum_power_used
    }
}
...
mod tests {
    ...
    fn colluding_instances_exceed_coalition_bound_blocked() {
        ...
        assert!(
            res.is_err(),
            "coalition ceiling must block colluding instances"
        );
        let err = res.unwrap_err();
        assert_eq!(err.axis, "compute");
        assert!(
            err.rollup + err.attempted > err.ceiling,
            "anti-vacuous: denied attempt is over the ceiling"
        );
    }
    ...
    fn sockpuppet_majority_cannot_force() {
        ...
        assert!(
            r.is_err(),
            "sockpuppet majority must be blocked at quorum ceiling"
        );
