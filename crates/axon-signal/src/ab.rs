...
pub fn compute_ab_report(ledger_dir: &Path, store: &Store) -> anyhow::Result<AbReport> {
    ...
    let mut outcomes: Vec<RecommendationOutcome> = recs
        .iter()
        .map(|rec| {
    ...
    outcomes.sort_by_key(|b| Reverse(b.rec.recorded_ms));
    ...
    for o in &outcomes {
        ...
        if let Some(d) = o.delta {
            entry.1.push(d);
            if o.improved == Some(true) {
                entry.2 += 1;
            }
        }
    }

    let mut by_type: Vec<AbTypeSummary> = outcomes
        .iter()
        .map(|o| o.rec.rec_type.clone())
        .collect::<std::collections::HashSet<_>>()
        .into_iter()
        .map(|rt| {
            let key = format!("{:?}", rt);
            let (instances, deltas, improved_count) =
                type_map.get(&key).cloned().unwrap_or((0, vec![], 0));
            ...
            let improved_rate = if instances_with_followup == 0 {
                None
            } else {
                Some((improved_count as f64 / instances_with_followup as f64 * 100.0).round())
            };
            AbTypeSummary {
                rec_type: rt,
                instances,
                instances_with_followup,
                avg_delta,
                improved_rate,
            }
        })
...
const AB_DISCLAIMER: &str =
    "This is an observational before/after measurement per recommendation week, \
...
mod tests {
    ...
    fn test_rec_type_detection() {
        assert_eq!(
            RecommendationType::detect("add file refs to goals"),
            RecommendationType::GoalClarity
        );
        assert_eq!(
            RecommendationType::detect("scope to one function"),
            RecommendationType::ScopeControl
        );
        assert_eq!(
            RecommendationType::detect("define exit criteria"),
            RecommendationType::ReworkPrevention
        );
        assert_eq!(
            RecommendationType::detect("commit partial progress"),
            RecommendationType::CommitRate
        );
        assert_eq!(
            RecommendationType::detect("use /loop to automate"),
            RecommendationType::LoopAdoption
        );
        assert_eq!(
            RecommendationType::detect("something unrelated"),
            RecommendationType::Other
        );
    }
    ...
    fn test_record_and_load() {
        ...
        let rec =
            record_recommendation(dir.path(), "2026-W25", "add file refs to goals", 62.0, None)
                .unwrap();
    ...
    fn test_render_ab_with_data() {
        let report = AbReport {
            outcomes: vec![RecommendationOutcome {
