...
pub fn emit(goal: &GoalFile) -> Result<String> {
    ...
    if !used_prelude.is_empty() {
        let _ = writeln!(
            out,
            "// ── Prelude (auto-included helpers the goal references) ──"
        );
    ...
    if default_loop && !provides("test_input") {
        ...
        let _ = writeln!(
            out,
            "// TODO: fill in N concrete test cases (3-5 typical, 1-2 adversarial)."
        );
    ...
    if default_loop && !provides("adversarial_input") {
        ...
        let _ = writeln!(
            out,
            "    // TODO: hallucination-bait / prompt-injection / edge-case input."
        );
    ...
    if default_loop && !provides("build_prompt") {
        ...
        let _ = writeln!(
            out,
            "// TODO: enumerate 4-8 distinct prompt-prefix variants to compare."
        );
    ...
    if default_loop && !provides("score_output") {
        ...
        let _ = writeln!(
            out,
            "// TODO: implement the score formula from the Score section (0..100)."
        );
        let _ = writeln!(
            out,
            "fn score_output({inputs_arg_name}: {inputs_arg_ty}, {output_arg}) -> i64 {{"
        );
        let _ = writeln!(out, "    0");
        let _ = writeln!(out, "}}");
        let _ = writeln!(out);
    ...
    if !provides("try_variant") {
        ...
        if let Some(contained) = goal.contained_attr() {
            let _ = writeln!(
                out,
                "// effect surface declared in prose — compiler-enforced:"
            );
    ...
    let _ = writeln!(
        out,
        "    println(\"goal: {} — searching variants...\")",
        goal.title
    );
    ...
    if provides("feasible") {
        let _ = writeln!(
            out,
            "    // constrained by the author's `feasible` predicate (prose Constraints)"
        );
        let _ = writeln!(out, "    let result = goal_run_constrained(\"try_variant\", \"feasible\", {target}.0, {budget})");
    } else {
        let _ = writeln!(
            out,
            "    let result = goal_run(\"try_variant\", {target}.0, {budget})"
        );
    }
    let _ = writeln!(out, "    let best = f64_to_i64(result)");
    let _ = writeln!(
        out,
        "    println(\"best score: {{to_str(best)}} (target {target})\")"
    );
...
"#;
    ...
    fn redteam_rejection_exits_with_the_policy_code_3() {
        ...
        let ax = emit(&g).unwrap();
        assert!(
            ax.contains("if redteam_check() {"),
            "redteam gate must be wired: {ax}"
        );
    ...
    fn feasible_predicate_wires_constrained_search() {
        ...
        let ax = emit(&g).unwrap();
        assert!(
            ax.contains("fn feasible(variant_id: i64) -> bool"),
            "feasible lifted: {ax}"
        );
        assert!(
    ...
    fn prose_effect_surface_is_compiler_enforced_via_contained() {
        ...
        let ci = ax
            .find("@[contained")
            .expect("contained stamped on the loop");
        let ai = ax
            .find("@[adaptive]")
            .expect("adaptive try_variant present");
        assert!(
            ci < ai,
            "@[contained] must precede the @[adaptive] loop: {ax}"
        );
        assert_eq!(
    ...
    fn bundles_only_referenced_prelude_helpers() {
        ...
        assert!(
            ax.contains("fn normalize_score("),
            "referenced helper not bundled"
        );
        assert!(
            !ax.contains("fn weighted2("),
            "unreferenced helper should not be bundled"
        );
    }
