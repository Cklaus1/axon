...
pub fn handle(mut req: Request, axon_bin: &str) {
    ...
    let (status, body, ct) = match (method.as_str(), url.as_str()) {
        ("GET", "/") | ("GET", "/index.html") => {
            (200, crate::html::INDEX_HTML.to_string(), "text/html; charset=utf-8")
        }
        ("POST", "/api/intent/compile") => {
            let b = read_body(&mut req);
            (200, crate::api::intent_compile(&b, axon_bin), "application/json")
        }
        ("POST", "/api/ast/review") => {
            let b = read_body(&mut req);
            (200, crate::api::ast_review(&b, axon_bin), "application/json")
        }
        ("POST", "/api/ast/approve") => {
            let b = read_body(&mut req);
            (200, crate::api::ast_approve(&b, axon_bin), "application/json")
        }
        ("POST", "/api/redteam") => {
            let b = read_body(&mut req);
    ...
    let ct_hdr = Header::from_bytes(b"Content-Type", ct.as_bytes())
        .expect("valid header");
    let cors_hdr = Header::from_bytes(b"Access-Control-Allow-Origin", b"*")
        .expect("valid header");
