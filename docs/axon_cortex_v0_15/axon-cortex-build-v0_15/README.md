...
**Prepared:** 2026-09-19. **Revision:** v0.15 evidence graph, causal experimentation and cognitive scheduling refinement. **Status:** Draft architecture proposals. **Product implementation/tests:** not performed in this package.
...
| [Specification index](specs/INDEX.md) | 35 detailed Draft specs with dependencies, contracts and negative acceptance cases. |
| [Build plan](build/BUILD_PLAN.md) | Milestones M0–M9, intent-first entry path, parallel tracks and the first vertical slice. |
| [Build tasks](build/TASKS.md) | 152 dependency-tracked work packages; optional research/native/kernel branches are explicit. |
...
| [Acceptance gates](build/ACCEPTANCE_GATES.md) | 247 proposed product gates; none is presented as already executed. |
...
| [Coding Frontier Lab](build/CODING_FRONTIER_LAB.md) | Protected whole-system coding benchmark, transfer/frontier curves and regression evidence. |
| [Cognitive specialization](build/COGNITIVE_SPECIALIZATION.md) | Recurring-function mining, matched specialization bakeoffs, applicability, ROI, drift and fallback. |
| [Neural programs](build/NEURAL_PROGRAMS.md) | Typed learned-function artifacts, shared-base runtime, offline/local execution and crystallization path. |
...
| [Decision composition](build/DECISION_COMPOSITION.md) | SELECT/PROJECT/COPY/COMPOSE typed world artifacts before open-ended generation. |
| [Semantic alignment loop](build/SEMANTIC_ALIGNMENT_LOOP.md) | Version and improve questions, criteria, decompositions and candidate policies independently of model weights. |
| [Semantic supervisor plane](build/SEMANTIC_SUPERVISOR_PLANE.md) | Independent progress/drift/stuck/verification assessment with deterministic interventions and no completion authority. |
| [Semantic working-set manager](build/SEMANTIC_WORKING_SET_MANAGER.md) | Protected context pinning, semantic GC, recompute contracts, dynamic rule/skill selection and cognitive cascades. |
| [Reflexive self-application](build/SELF_APPLICATION.md) | Internal cognitive-operation telemetry, replay/shadow challengers, protected-kernel boundaries, canary/rollback and new-primitive proposals. |
| [Repository optimization](build/REPOSITORY_OPTIMIZATION.md) | Project contracts, baselines, isolated challengers and project-local promotion/rollback. |
| [Cross-project learning](build/CROSS_PROJECT_LEARNING.md) | Family-aware pattern mining, held-out transfer, scope-qualified shared capabilities and de-generalization. |
| [Universal evidence graph](build/UNIVERSAL_EVIDENCE_GRAPH.md) | Typed provenance from intent and observations through actions, claims, verification, admission and rollback. |
| [Causal/active experimentation](build/CAUSAL_ACTIVE_EXPERIMENTATION.md) | Matched interventions, causal assumptions, information-gain experiment selection and reversibility-aware authority. |
| [Cognitive scheduler & registry](build/COGNITIVE_SCHEDULER_REGISTRY.md) | Authorized shared capability registry plus utility/hardware/cache/risk-aware cognitive dispatch. |
...
See [SELF_APPLICATION](build/SELF_APPLICATION.md) and [CX-29](specs/CX-29-reflexive-self-application-plane.md).
...
This revision connects the previously separate control/learning planes with three platform primitives. CX-32 provides one Universal Evidence Graph so every consequential claim, replay, challenger, admission and rollback has typed traversable provenance. CX-33 adds causal and active experimentation so Cortex can choose interventions for information gain and distinguish association from mechanism. CX-34 creates a Shared Capability Registry and Cognitive Scheduler so rules, semantic matchers, specialized/general Reflexes, Neural Programs, generators, tools and human escalation are dispatched under hard authority/privacy/type constraints plus explicit quality/latency/cost/risk/hardware/cache objectives.
