...
## v0.15 review additions

Three connective gaps are promoted to first-class specs: evidence interoperability, causal improvement claims, and heterogeneous cognitive dispatch. Remaining high-leverage follow-ons—knowledge garbage collection, adversarial challenger generation, architecture-drift linting, protocol-native multi-agent roles, human-feedback gold events and an improvement-portfolio manager—remain candidate extensions rather than silently expanding this revision's implementation surface.
