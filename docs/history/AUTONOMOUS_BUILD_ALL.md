...
8. **R10 — self-improving harness.** Spec ✅ Reviewed. ⚠️ R10→85%: **ALL FOUR GATES + GRADUATION FIREWALL BUILT.** 0ffd35e: G1 (oracle, E1401, E1406-by-construction) + G2 (cap-diff I-12, E1402) + `run_program_capturing`. 2637ae5: G3 (@[test] outcome, E1403) + G4 (opt-in perf, W1410). This commit: `manifest.rs` (content-addressed `passes.manifest`, multi-sig `graduate` gate E1404 — ≥2 DISTINCT root Principals, duplicate-signer refused; malformed E1405; `is_graduated`=execution permission; `revert`) + `axon improve` CLI (verify/graduate/list/revert; `discover` honest-blocks). Only `discover` (unprivileged AI proposal of candidate passes) remains. The verification core + human-gated graduation — the safety-critical parts — are complete.
