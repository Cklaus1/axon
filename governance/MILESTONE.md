...
| **R6** | Capability security (lockfile, audit, `@[sensitive]`) | 96 | ✅ **all four taint paths closed + composing** — INTERPROCEDURAL (helper forwards to a sink, fixpoint `exfiltrating_params`), LOCAL (`let e = u.email`, `sensitive_locals` w/ rebind-clears), RETURN-VALUE (`let e = extract(u)`, fixpoint `taint_returning_params`), CONTAINER-STORE (`let w = Wrapper{data:u.email}; sink(w.data)`). All E1206; a full `extract→let→relay→sink` chain caught end-to-end. AI-audit quality remains |
