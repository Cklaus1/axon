...
| Req | What | % | State |
|---|---|---|---|
| **R1** | Native pipeline (parse→type→borrow→LLVM→native) | 98 | ✅ 32/32 example parity; **stdlib codegen coverage now ~100 builtins — the arr_* family is COMPLETE (reduction/alloc/closure incl. f64-key max_by/min_by via bitcast-transport/tuple/nested + index_of/take_while/drop_while/push) and the dict family is ~75% (new/set/get/has/len/inc/get_or/remove/keys/values/merge/from_pairs/to_pairs/map_values/to_str), plus exp/ln/log10 + str_split/join/digits_only — all native==interp==AOT-wasm**; a differential parity FUZZER (fuzz_parity.sh, 37 seeded descriptors + NaN/inf/overflow boundary cases) + dict/arr parity harnesses auto-guard I-2 in the gate. Three reusable codegen patterns landed: bitcast-transport (non-i64 lambda returns), repr(C)-layout-match (composite tuple args), runtime-callback (closures invoked from inside axon-rt). The last unimplemented builtins fail honestly (E0910) and run under interp — they hit VERIFIED design-level walls: dict_filter/each need a str-PARAMETER lambda ABI (lambdas are i64-param-hardwired; a 10-callsite refactor), dict_from_str needs str-valued dict_get, arr_group_by needs both |
