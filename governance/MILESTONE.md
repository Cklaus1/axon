...
| **R6** | Capability security (lockfile, audit, `@[sensitive]`) | 94 | ✅ **taint now closed both ways** — INTERPROCEDURAL (a sensitive value laundered through a helper into an AI/file/exec sink, fixpoint `compute_exfiltrating_params`) AND LOCAL (a sensitive field copied to a local `let e = u.email; sink(e)`, per-fn `sensitive_locals` with rebind-clears-taint); both E1206. AI-audit quality remains |
