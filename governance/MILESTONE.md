...
| **R3** | AI as a language primitive (routing, policy, budget, cost) | 89 | ✅ tier routing + live token-count + per-token cost meter live; **`Budget<R...>` Tier-1 type landed userland** (`ResBudget` — multi-axis calls/tokens/cost, ANY-axis-overrun exhausts); `tier:` accepts a string value. Kernel generic-over-R Budget + per-ai_complete runtime tick = Phase-7 |
| **R4** | Three code zones + compiler-enforced provenance | 100 | ✅ Complete; **I-13 agent action log hardened** — now logged transitively through helper calls (was escapable by wrapping the I/O one call away), via the interp's `enclosing_agent` attribution. Native-codegen transitive log is a documented follow-on (needs runtime agent-context threading) |
| **R5** | Goal-directed optimization (5 strategies, held-out eval) | 97 | ✅ kernel `Goal` landed (fdbb8b2, R12b) — principal-budget-scoped, exit-7 on exhaust; language-level `Goal<M>` value-type sugar deferred |
| **R6** | Capability security (lockfile, audit, `@[sensitive]`, `@[contained]`) | 97 | ✅ **@[sensitive] taint closed all 4 paths** (interprocedural/local/return-value/container-store, all E1206) AND **@[contained] sandbox now enforced TRANSITIVELY** (a contained fn calling a helper that does the forbidden exec/net/write is E1001 — the I-11 TCB escape closed; `CapCtx` follows helper calls under the caller's spec, visited-set stops recursion). Path/host-allowlist still needs a literal arg; AI-audit quality remains. **FURTHER HARDENED (2026-06-10):** @[sensitive] taint now also closes value-DERIVATION launders (through str builtins / interpolation / binops / if-match-block results, 688abec/64346e2) AND exfiltration through a METHOD (3ed4e24); @[contained] dynamic (non-literal) I/O paths now FAIL CLOSED (E1001), not open (0f12562). |
...
| **R9** | Layer-1/3 alignment (`Uncertain`/`Temporal`/`@[verify]`/causal) | 93 | ✅ **full `Uncertain`/`Temporal` PARITY across every layer** — soft-typing (if/while cond, scalar param/return, binop), field surface (source_tag/valid_until_ms phantom fields fixed), AND `@[verify]` (scalar + Uncertain + Temporal, value + confidence, static-classify + runtime-gate, both engines). A soft wrapper into a plain-T slot unwraps; a verify bound on either wrapper is enforced (exit 3 on breach). native==interp throughout. SMT loop invariants + full metacognition trait remain |
