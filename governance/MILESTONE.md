...
| **R9** | Layer-1/3 alignment (`Uncertain`/`Temporal`/`@[verify]`/causal) | 90 | ✅ **`@[verify]` now enforced on SCALAR returns** (i64/f64/bool) at runtime — was silently unenforced unless the return was `Uncertain<T>`, a real hole for a hard bound like `@[verify(value <= 500)]`; fixed in BOTH interp + native codegen (native==interp, exit 3 on breach). SMT loop invariants + full metacognition trait remain |
