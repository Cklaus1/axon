...
| **R6** | Capability security — content-addressed imports, compile-time I/O restriction, AI audit on import | Supply Chain Security; Capability Permissions | 30 | ⚠️ Partial | `@[contained]` enforces net/fs/exec at fn boundary (CLI-gated). **Import-edge boundary VERIFIED (I-11)**: the static capability checker runs on the merged post-import program, so a `mod`+`use`-imported fn that violates its `@[contained]`/`never:` is caught (E1004) — the hard security floor holds across the import edge regardless of any AI audit (cli_run test). Still no registry, no content-addressing (`axh1:` hash + `axon.lock`), no `--locked` tamper check (E1201), no import audit. | 📝 `governance/specs/R6-capability-security.md` (Reviewed) | Import without declared capability fails compile ✅ (E1004 across import edge); tampered content hash rejected (pending — lockfile machinery unbuilt) |
