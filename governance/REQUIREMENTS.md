...
| **R3** | AI as language primitive — `std.ai`, model routing, `#[ai(policy)]`, deterministic fallback | AI as Primitive; AI Policy | 55 | ⚠️ Partial | **AiCall provenance record DONE** (§4.3): every `ai_complete` appends one `event:"ai_call"` NDJSON record (tier/model/params_hash/**prompt_hash SHA-256, not verbatim**/mode/reason/cost_usd), deterministic (R9 replay key). **OFFLINE FALLBACK + E1300 DONE** (§3.3/§6): `@[ai(policy(fallback: "x"))]` parses (nested-group attr form + flat form) and, when offline, returns `Ok("x")` stamped `mode:"fallback"` — the program stays TOTAL; no fallback offline = coded **E1300** (not a generic panic). Still missing: `tier:` named-arg routing (tier→model table), budget gate (E1301, Phase-7 Budget), `axon ai policy` CLI. | 📝 `governance/specs/R3-ai-primitive.md` (Reviewed) | Policy-gated fallback fires offline ✅ (cli_run: fallback-returns + E1300); routing picks tier by cost (pending); **ai_call provenance recorded ✅** (record-shape + deterministic-hash) |
