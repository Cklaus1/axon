...
| **R6** | Capability security — content-addressed imports, compile-time I/O restriction, AI audit on import | Supply Chain Security; Capability Permissions | 48 | ⚠️ Partial | `@[contained]` enforces net/fs/exec at fn boundary. **Import-edge boundary VERIFIED (I-11)**: static cap checker catches a `mod`+`use`-imported fn's `@[contained]`/`never:` violation (E1004) on the merged program. **Content-addressed lockfile WORKING end-to-end**: `lockfile.rs` (zero-dep `axh1:` SHA-256 + deterministic `axon.lock`) wired to CLI — `axon lock <file>` hashes each `use`d module + writes the lock; `axon verify-lock <file>` recomputes and rejects a tampered module with **E1201** (exit 2), a missing entry with **E1202**. The PRD's "tampered content hash rejected" is delivered. Remaining: `--locked` on `axon check` (verify-lock already gates), E1203 cap-widening, AI audit pinning (needs R3), transitive-`use` hashing. | 📝 `governance/specs/R6-capability-security.md` (Reviewed) | Import without declared capability fails compile ✅ (E1004); **tampered content hash rejected ✅** (axon lock/verify-lock → E1201); audit on import (pending) |
...
| **R10** | Self-improving compiler — learned optimization passes graduated from AI-discovered asm | Self-Improving Compiler; Intelligence Maturity | 70 | ⚠️ Partial | **Verification harness — ALL FOUR GATES BUILT** (`improve.rs`). G1 correctness: observable tuple `(exit_code, stdout)` via the **interpreter oracle** (never AI — E1406 by construction) over the WHOLE corpus → output-changing/overfit pass rejected (E1401). G2 safety (I-12): `program_capabilities` diff → capability-widening pass rejected (E1402). G3 regression: each member's `@[test]` fns keep their pass/fail outcome under the pass → test-breaking pass rejected (E1403), catching what G1 (main-only) misses. G4 perf: opt-in wall-clock timing (`measure_perf`), faster-on-≥1/slower-on-none → `PerfStatus::Faster`, else `Unmeasured`/W1410 — advisory, gates only the `faster` *claim*. Deterministic. E14xx/W1410 band. Remaining: the `axon improve` CLI + content-addressed pass manifest + multi-sig graduation (E1404/E1405) — the graduation machinery atop a complete verification core. | 📝 `governance/specs/R10-self-improving-compiler.md` (Reviewed) | A discovered pattern verified correct (G1 ✅) + safe (G2 ✅) + non-regressing (G3 ✅) + faster (G4 ✅), graduation gated; auto-applied via CLI/manifest pending |
