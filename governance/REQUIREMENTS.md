...
| **R6** | Capability security — content-addressed imports, compile-time I/O restriction, AI audit on import | Supply Chain Security; Capability Permissions | 38 | ⚠️ Partial | `@[contained]` enforces net/fs/exec at fn boundary (CLI-gated). **Import-edge boundary VERIFIED (I-11)**: static cap checker catches a `mod`+`use`-imported fn's `@[contained]`/`never:` violation (E1004) on the merged program. **Lockfile FOUNDATION built** (`lockfile.rs`, zero-dep): `axh1:`-tagged SHA-256 over raw bytes + deterministic `axon.lock` TOML serializer/parser (sorted order, E1205 on malformed). E12xx/W1210 band defined. Still no CLI wiring (`axon lock`/`add`/`verify-lock`), no import-time `--locked` hash check (E1201), no audit. | 📝 `governance/specs/R6-capability-security.md` (Reviewed) | Import without declared capability fails compile ✅ (E1004 across edge); tampered content hash rejected (foundation ready — `axh1:` hash + lock round-trip ✅; `--locked` check pending) |
