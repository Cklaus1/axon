...
|---|---|---|---|---|---|---|---|
| **R1** | Native compiler pipeline at Tier-1 perf (parse→typecheck→borrow→LLVM→native) | Compiler Architecture; Build Phases | 40 | ⚠️ Partial | LLVM build does not finish (`BUILD_DIAGNOSIS.md`); interpreter is the live path. Native binary not routinely producible. | `spec/compiler-phase1.md` + `BUILD_DIAGNOSIS.md`; 📝 `governance/specs/R1-codegen-build-unblock.md` (Draft — move inline-IR builtins → axon-rt; wrappers from `CODEGEN_WRAPPER_PROTOTYPE.md` already shipped) | A native binary of `examples/*.ax` runs and matches interpreter output + a perf-tier benchmark |
