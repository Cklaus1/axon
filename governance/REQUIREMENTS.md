...
| **R3** | AI as language primitive — `std.ai`, model routing, `#[ai(policy)]`, deterministic fallback | AI as Primitive; AI Policy | 62 | ⚠️ Partial | **AiCall provenance record DONE** (§4.3): every `ai_complete` appends one `event:"ai_call"` NDJSON record (tier/model/params_hash/**prompt_hash SHA-256, not verbatim**/mode/reason/cost_usd), deterministic (R9 replay key). **OFFLINE FALLBACK + E1300 DONE** (§3.3/§6): `@[ai(policy(fallback: "x"))]` parses (nested-group attr form + flat form) and, when offline, returns `Ok("x")` stamped `mode:"fallback"` — the program stays TOTAL; no fallback offline = coded **E1300** (not a generic panic). **TIER ROUTING (policy+default) DONE**: `ai_routing.rs` (cheap/balanced/strong → concrete model+version host table); `@[ai(policy(tier: X))]` resolves the tier (default balanced), and the AiCall provenance now records the *real* routed `(tier, model, version)` instead of a hardcoded "balanced"/"mock" placeholder. Unknown tier → E1302; AI call with no `@[ai(policy)]` → W1310 (un-metered warning). Still missing: per-call `tier:` named-arg (needs call-grammar; deferred), budget gate (E1301, Phase-7 Budget), `axon ai policy` CLI. | 📝 `governance/specs/R3-ai-primitive.md` (Reviewed) | Policy-gated fallback fires offline ✅ (E1300); **tier routing pins real model ✅** (policy+default; E1302/W1310); per-call tier + cost routing (pending); **ai_call provenance recorded ✅** |
| **R4** | Three code zones: Static / Adaptive / Agent + compiler-enforced provenance | Three Code Zones; Error/Perf Logging | 75 | ⚠️ Partial | `@[adaptive]`/`@[experiment]`/`@[agent]` all behaviorally distinct now. `@[experiment]` injects tagged provenance (`zone:"experiment"`+label) but is excluded from `goal_run` best. **`@[agent]` MANDATORY ACTION-LOG DONE**: `call_builtin` injects an `event:"agent_action"` record (`zone:agent`, `action`=tool, `caps_used`=capability) whenever a capability-bearing builtin runs inside an `@[agent]` fn — compiler-injected, un-opt-out-able (I-13); non-agent fns doing the same call log nothing. JSONL carries real `event`/`zone`/`label`. Still missing: provenance not *un-opt-out-able* in codegen (R1 conformance tripwire). | `ROADMAP.md` §6–7; ✅ `governance/specs/R4-code-zones.md` (Reviewed — interp slice landed; codegen conformance R1-gated) | Adaptive fn cannot compile without provenance injection ✅ (zone injection engine-keyed, no opt-out); `@[experiment]` distinct ✅ (cli_run: distinct-from-adaptive + trace-survives); agent action log mandatory ✅ (cli_run: agent_actions_are_mandatorily_logged) |
...
| **R6** | Capability security — content-addressed imports, compile-time I/O restriction, AI audit on import | Supply Chain Security; Capability Permissions | 60 | ⚠️ Partial | `@[contained]` enforces net/fs/exec at fn boundary. **Import-edge boundary VERIFIED (I-11)**: static cap checker catches a `mod`+`use`-imported fn's `@[contained]`/`never:` violation (E1004) on the merged program. **Content-addressed lockfile WORKING end-to-end**: `lockfile.rs` (zero-dep `axh1:` SHA-256 + deterministic `axon.lock`) wired to CLI — `axon lock <file>` hashes each `use`d module + writes the lock; `axon verify-lock <file>` recomputes and rejects a tampered module with **E1201** (exit 2), a missing entry with **E1202**. The PRD's "tampered content hash rejected" is delivered. **Import-edge cap-widening DONE (E1203)**: a `@[contained]` importer that imports a module exercising a capability it doesn't grant is rejected at `axon check` (`check_import_capabilities`, opt-in: uncontained importers unaffected — back-compat). Remaining: AI audit pinning (needs R3), transitive-`use` hashing. | 📝 `governance/specs/R6-capability-security.md` (Reviewed) | Import without declared capability fails compile ✅ (E1004 + **E1203 import-edge**); **tampered content hash rejected ✅** (axon lock/verify-lock → E1201); audit on import (pending) |
| **R7** | Cross-platform targets — native / wasm / js / mobile from one source | Cross-Platform; Mobile; UI; 3D | 35 | ⚠️ Partial | **Slice A DONE — Axon runs on wasm.** The tree-walking interpreter compiles to `wasm32-wasip1` and runs `.ax` headless under `wasmtime` with **identical exit codes + stdout to native on 15/15 pure-compute examples** (same `interp.rs`, two targets — identical by construction, I-2). One host touchpoint changed: `on_deep_stack` runs on the single wasm stack (`#[cfg(wasm32)]`); 64 MiB wasm stack (`.cargo/config.toml`) + `RECURSION_LIMIT=450` so deep recursion fails with the same graceful panic as native, no trap. Remaining: `AxonHost` trait for `unknown-unknown`/browser virtual-FS, `axon target` CLI, js/mobile, AOT-wasm (Slice B). | 📝 `governance/specs/R7-targets.md` (Reviewed — Slice A landed) | Same `.ax` runs on native and wasm with identical observable results ✅ (15/15 pure-compute, `scripts/wasm_parity.sh`) |
