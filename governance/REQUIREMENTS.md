...
| **R10** | Self-improving compiler — learned optimization passes graduated from AI-discovered asm | Self-Improving Compiler; Intelligence Maturity | 55 | ⚠️ Partial | **Verification harness — the two safety-critical gates BUILT** (`improve.rs`). G1 correctness: `verify_pass` compares the observable tuple `(exit_code, stdout)` via the **interpreter oracle** (`run_program_capturing`, never an AI judgment — E1406 by construction) over the WHOLE corpus, so an output-changing or overfit pass is rejected (E1401). G2 safety (I-12): diffs `program_capabilities` — a pass that adds `fs:read`/`fs:write`/`net`/`exec` is rejected (E1402). Deterministic records. E14xx band defined. Remaining (implementation, same spec): G3 regression-suite gate (E1403), G4 native timing harness, `axon improve` CLI + multi-sig graduation manifest (E1404/E1405). | 📝 `governance/specs/R10-self-improving-compiler.md` (Reviewed) | A discovered pattern verified correct (G1 oracle ✅) + safe (G2 caps ✅), graduation gated; faster (G4 timing) + auto-applied (CLI/manifest) pending |
