...
|---|---|---|---|---|---|---|---|
| **R1** | Native compiler pipeline at Tier-1 perf (parse→typecheck→borrow→LLVM→native) | Compiler Architecture; Build Phases | 75 | ✅ Working | **Build SOLVED** (`BUILD_RESOLVED.md`): the stall was a serde×codegen default-feature collision, not LLVM/inkwell. `cargo build --no-default-features --features codegen --bin axon` finishes in ~3s; `axon build examples/*.ax` produces native binaries matching the interpreter (5/5 verified). Remaining: move serde-json out of default so the plain build works (BUILD_RESOLVED §3), Tier-1 perf benchmark. | `spec/compiler-phase1.md`; `BUILD_RESOLVED.md` (resolution); `BUILD_DIAGNOSIS{,_2,_3}.md` (the diagnostic journey) | A native binary of `examples/*.ax` runs and matches interpreter output ✅ + a perf-tier benchmark (pending) |
