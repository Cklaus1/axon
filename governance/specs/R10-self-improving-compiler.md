...
- [x] `graduation_requires_multisig` passes (E1404 — I-12 governance). **DONE** — `manifest.rs::graduate` refuses (E1404) unless ≥2 *distinct* root Principals sign; a duplicate signer (self-graduation by signing twice) is rejected too. `axon improve graduate --sign A --sign B` is the CLI; the `passes.manifest` is content-addressed (`axp1:`/`axv1:`/`axc1:`) and a pass not in it never runs (`Manifest::is_graduated` = execution permission). `list`/`revert` round-trip (gate-3 reversibility). Tests: manifest `graduation_requires_multisig` + cli_run `improve_graduate_requires_multisig_e1404` / `improve_list_and_revert_roundtrip`.
...
**All four gates AND the graduation machinery are now built and tested.** The four gates (G1 oracle, G2 capability firewall, G3 regression, G4 perf) plus the graduation firewall — `manifest.rs` (content-addressed `passes.manifest`, multi-sig `graduate` gate E1404, malformed-manifest E1405, `is_graduated` = execution permission, `revert` reversibility) and the `axon improve` CLI (`verify`/`graduate`/`list`/`revert`, with `discover` an honest block — it is the unprivileged AI proposal side, the one remaining slice). R10 rose **0% → ~85%** across three slices. The verification core and the human-gated graduation path — the parts that had to be right before any discovery machinery exists — are complete. The only open item is `discover` (AI proposal of candidate passes), which is unprivileged by design: a proposal grants nothing; only `graduate` (multi-sig) grants execution.
