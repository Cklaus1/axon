...
- [x] `tier_resolution_prefers_call_over_policy_over_default` passes — **partial: policy + default DONE** (`ai_routing.rs` Tier enum + host model table; `current_ai_tier` resolves from `@[ai(policy(tier: X))]`, default `balanced`). The AiCall provenance now records the *resolved* tier + concrete `(model, version)` instead of a hardcoded placeholder (cli_run `ai_tier_routing_pins_real_model_in_provenance`). Unknown tier → **E1302** (`unknown_ai_tier_is_e1302`); a no-policy AI call → **W1310** (`ai_call_without_policy_warns_w1310`). *Per-call `tier:` arg (step 1) is deferred — it needs named-argument call grammar, which doesn't exist yet; policy-tier + default (steps 2-3) is the shippable routing.*
...
- [x] `axon ai policy` emits one JSON line per @[ai] fn (fn/tier/fallback/model from the ai_routing table, **plus `budget`**). **DONE** (cli_run: `ai_policy_prints_resolved_policy_json`). `budget` was added later (cli_run: `ai_policy_reports_the_budget_that_is_actually_enforced`): this list was written while the budget slice below was still open, so the reporter shipped without the one field that decides whether an `ai_complete` is FATAL — two fns differing in E1301 enforcement printed identical lines. It reports three distinct states: `null` (no `budget:`, unmetered), a number (the ceiling), and `"invalid"` (malformed — W1311 at runtime, runs unmetered; NOT folded into `null`, which would present a disarming typo as a deliberate choice). Read via the shared `ai_routing::budget_from_attrs`, so the reporter, the interpreter's meter and the native codegen refusal cannot disagree about which fns are metered.
