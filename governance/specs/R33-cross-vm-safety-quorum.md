...
protocol (not just the Rust test helpers). S2e+ (the `propose --broadcast` flag — the first real,
non-test caller of `broadcast_and_collect` — and real `AF_VSOCK`) still open. See §14 Evidence
ledger.
...
| R33.S2e+ (`propose --broadcast` CLI flag + real `AF_VSOCK`) | R33.S2d | not yet named | todo — the `axon-vm quorum propose --broadcast CID1,CID2,...` flag (gives `broadcast_and_collect` its first non-test caller) and real `AF_VSOCK` (swapping S2b/S2c/S2d's TCP-loopback stand-in) |
