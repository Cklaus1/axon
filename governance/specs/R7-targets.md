...
**Status:** 📝 Draft (2026-06-01) — *partially blocked on R1; the unblocked path is specified, the blocked path is scoped.*
**Requirement:** `../REQUIREMENTS.md` R7 — *Cross-platform targets: native / wasm / js / mobile from one source.*
**Decisive fork (from `README.md`):** *Does wasm reuse the LLVM backend or need a separate codegen?* (a) native+wasm share the inkwell path (blocked on R1's stalled build), or (b) wasm gets its own lean backend. *"The native build must be unstalled (R1) before any multi-target work is real."* **→ Resolved below, with a third option the fork omitted.**

---

## 1. Motivation

R7 is 10% (❌ Thin): native only, and native is itself stalled (R1 — `BUILD_DIAGNOSIS.md`: the cost is *100% in the LLVM codegen backend*, single-threaded IR/object emission for the inkwell-heavy `codegen::builtins`). wasm/js/mobile = 0%.

The fork as written is a **false binary**. It assumes wasm must come from a *codegen* backend (LLVM-IR→wasm or a hand-written wasm emitter). But Axon has a **second, already-shipped execution engine that is pure Rust with no LLVM**: the tree-walking interpreter (`interp.rs`, the `--no-default-features` build, `Cargo.toml` `[[bin]] axon-run`). Compiling *the interpreter itself* to `wasm32-unknown-unknown` yields an in-browser Axon runtime that executes `.ax` source directly — and this path **does not touch the stalled LLVM backend at all** (I-2: the interpreter is already the reference semantics).
...
- **(B) LLVM→wasm codegen** — reuse the inkwell pipeline targeting `wasm32-wasi`. **R1-blocked** (same backend stall). The existing cross-compile surface (`codegen/link.rs` `target_triple`, `cross.toml`, E0904/E0905) is the seam.
...
Critically, "**runs**… identical observable results" is satisfiable by **option (A)** — the interpreter compiled to wasm runs the same `.ax` and, being the *same code* as the native interpreter, produces identical results **by construction** (I-2). "**compiles**" in the AOT sense (a standalone `.wasm` *binary* of a program, not the interpreter) is option (B), R1-blocked. This spec splits the acceptance accordingly (§9).

Dependencies: **R1** (native build — blocks B/C), **I-2** (interpreter is reference — the lever that unblocks A), **I-11** (capability boundary — wasm's sandbox interacts with `@[contained]`, §4.4).
...
### 3.2 Option B — AOT wasm binary (R1-gated, specified-not-built)

```
axon build prog.ax --target wasm32-wasi        # reuses codegen/link.rs target_triple seam
# BLOCKED until R1 lands (BUILD_DIAGNOSIS.md). Errors today with E0907 (§6).
```

### 3.3 The host-capability boundary (option A's real work)

The interpreter calls `std::fs`, `std::env`, `std::thread` (`interp.rs`: `read_file`/`write_file` ~2141, env ~3441, `sleep` ~3453, `on_deep_stack` thread-scope ~283). wasm32-unknown-unknown has none of these. The surface is a **host interface trait** the program's I/O builtins route through:
...
| `.ax` using threads / `spawn` / channels | wasm32-unknown-unknown is single-threaded; the cooperative interpreter scheduler already runs spawns eagerly on one thread (it is not OS-threaded), so this works — but `on_deep_stack`'s `std::thread::scope` must be replaced (§4.3). |
| `.ax` using `sleep_ms` | No-op or host-provided async on wasm; spec'd as host-delegated (§4.3). |
| AOT `axon build --target wasm32-wasi` (option B) | **E0907** today (R1-blocked); becomes real when R1 lands. |
...
- **Provenance log (`append_provenance_jsonl`, fs-backed):** on wasm, writes to an in-memory host buffer the JS side can read — provenance is preserved (R4), just not on a real filesystem.
...
|---|---|---|
| **E0907** | `axon build --target wasm32-wasi` (AOT option B) while R1 codegen is unavailable/stalled | `` AOT wasm build needs the native codegen backend, which is not available (see BUILD_DIAGNOSIS.md); use `axon target build --engine interp --target wasm32` to run via the interpreter `` |
...
- [ ] **Integration:** `cargo build --target wasm32-unknown-unknown --no-default-features --bin axon-run` succeeds (the build itself is the first gate — it fails today on `std::thread::scope`).
...
- [ ] **Parity (interp↔codegen):** N/A for option A (it *is* the interpreter). Option B's native-vs-wasm-codegen parity is R1-gated and out of scope.
...
**Slice B (AOT wasm codegen — R1-gated, NOT claimable now):**
- [ ] *Blocked.* Requires R1's native build to land. Acceptance deferred to a follow-spec once `BUILD_DIAGNOSIS.md`'s fix (`CODEGEN_WRAPPER_PROTOTYPE.md`) is validated.
...
Option A is the *interpreter* — no Tier-1 perf claim (that's R1/option B's job). The honest framing: A delivers **portability and reach** (run anywhere wasm runs), not native speed. A perf budget applies only to option B (AOT), which is R1-gated. Stated so A is not oversold as a performance target.
...
- **Slice B/C:** not rolled out — specified and gated. Option C (lean backend) only opens if R1 stays blocked past a decision point; it would carry a full I-2 parity-test burden (a second engine) and is explicitly the least-preferred.

## 12. Open questions

Blocking the blocked slice (honestly out of our hands now):
- **Q1 (R1 native build):** Slice B/C cannot be real until `cargo build -p axon-core` (codegen) finishes. `BUILD_DIAGNOSIS.md` localizes the stall to LLVM backend emission; `CODEGEN_WRAPPER_PROTOTYPE.md` proposes a fix that is *prototyped but unvalidated*. **This is the gate for AOT wasm, js, and mobile.** Until it lands, R7 = interpreter-on-wasm only. Stated plainly per the honesty rule.
