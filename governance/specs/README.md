...
|---|---|---|---|
| `R3-ai-primitive.md` | R3 — AI as primitive | 🔲 Stub | **Reproducibility vs. capability.** Does `#[ai(policy)]` + model routing record enough to *replay* a run deterministically (PRD §AI Reproducibility)? If yes, every AI call must pin (model, version, seed, prompt-hash) into provenance — which forces the provenance schema (overlaps R4). Resolve the schema before the routing. |
