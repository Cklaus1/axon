...
| `R4-code-zones.md` | R4 — code zones + provenance | 📝 Draft | **How is provenance made un-opt-out-able (I-13), engine-uniform?** *Resolved:* injection is a compiler pass keyed on the zone annotation (as `interp.rs:665` already does for `@[adaptive]`), shared by both engines so neither can skip it; there is no opt-out construct (rejected by design). `@[experiment]` becomes behaviorally distinct (tagged, excluded from `goal_run` best) instead of a no-op synonym; `@[agent]` gets mandatory action logging. Interpreter slice testable now; codegen conformance is an `#[ignore]`d parity tripwire until R1. New E15xx band; completes I-13. |
