...
check() {
  ...
  if ! berr="$("$AXON" build "$WORK/$name.ax" -o "$WORK/$name" 2>&1)"; then
    # "unavailable" was asserted, not proved: a closure-capture codegen bug
    # printed this same line and the harness still summarised PASS.
...
if [ "$ran" -eq 0 ]; then
  harness_skip closure_capture_parity "nothing ran (native codegen unavailable for every case)"
fi
